...
Frame RTException__CurFrame (); /* defined in RTExceptionASM.s */
...
{
  Proc p = RTException__Find (callee->pc);
  Frame f;
  int i;
  int n;
  jmp_buf* context;
...
void RTException__Unwind (Frame *target)
{
  ...
  target_proc = RTException__Find (target->pc);
  dprintf ((stderr, "target=%s  pc=%x  sp=%x\n",
	    RTException__ProcName (target_proc), target->pc, target->sp));

  f = RTException__CurFrame ();
  p = RTException__Find (f.pc);
  if (!p) return;
