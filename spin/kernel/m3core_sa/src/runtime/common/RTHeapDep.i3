 ...
 *	Brought back the original SRC way of calculating the maximum
 *	page number.  Moved to runtime/common.
 *
 * 13-Nov-95  Przemek Pardyak (pardy) at the University of Washington
 *	Fixed off-by-one in MaxPageNo.
 *
 * 15-Aug-95  Przemek Pardyak (pardy) at the University of Washington
 *	Exported statically the maximum page number and the number of bits 
 *	to represent the maximum page number. Changed REAL-s into 
 *      INTEGER-s.
 *)
...
CONST
  ...
  MaxPageNo	  = 16384-1;			   (* SPIN *)
  LogMaxPageNo    = 14;    (* number of bits for a page number *) (* SPIN *)
...
PROCEDURE ProtectAddr (start, end : ADDRESS;
                       readable, writable: BOOLEAN);
(* ProtectAddr changes the protection on the range of addresses between
   start and end according to the readable and writable flags. The actual
   protected range will be rounded down and up to page boundries *)
...
(* Remaps the page frame currently mapped to address src to address dest.  
   The original mapping is removed *)
