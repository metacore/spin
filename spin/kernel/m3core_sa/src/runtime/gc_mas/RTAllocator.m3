...
IMPORT Cstdlib, RT0, RT0u, RTMisc, RTOS, RTType, Word, RTHeapRep, RTOSMachine;
IMPORT RTIO, RTSynch;

FROM RTHeapRep IMPORT Header, RefHeader, AllocForNew, Malloc;
FROM RTHeapRep IMPORT verbose, doSanity, CheckSanity;
...
PROCEDURE Allocate (defn: ADDRESS): REFANY =
  ...
  BEGIN
    RTOS.LockHeap();
    (*
    IF RTRefCount.inCollector THEN
      RTOS.Crash();
    END;
    *)
    (* IF verbose = 3 THEN RTIO.PutText("<A"); END;*)
    IF verbose > 3 THEN RTIO.PutText("Allocate\n"); END;
    IF doSanity AND NOT CheckSanity("start of Allocate") THEN 
      RTOS.Crash(); 
    END;
    BEGIN
      WITH z = RT0u.alloc_cnts[tc] DO z := Word.Plus (z, 1) END;
      res := AllocForNew(def.dataSize, def.dataAlignment);
      WITH header = LOOPHOLE(res - ADRSIZE(Header), RefHeader) DO
        header.typecode := tc;
        header.spare := 0;
        header.Z := FALSE; 
        header.V := FALSE; 
      ...
      END;
    END;
    IF doSanity AND NOT CheckSanity("end of Allocate") THEN RTOS.Crash(); END;
    IF verbose > 3 THEN RTIO.PutText("Allocate done\n"); END;
    (* IF verbose = 3 THEN RTIO.PutText("A>"); END;*)
    RTOS.UnlockHeap();

    (* for reference counting *)
    IF RTHeapRep.RefCount THEN
      RTMachineCollectorExtern.AssignKnown (res, LOOPHOLE(-1, ADDRESS));
    END;
...
PROCEDURE AllocateOpenArray (defn: ADDRESS; READONLY s: Shape): REFANY =
  ...
  BEGIN
    ...
    END;
    (*
    IF NOT CheckSanity("end of AllocateOpenArray") THEN RTOS.Crash(); END;
    *)
    IF verbose = 1 THEN RTIO.PutText("O>"); END;
    IF verbose > 1 THEN RTIO.PutText("AllocateOpenArray done\n"); END;
    RTOS.UnlockHeap();
    (*
