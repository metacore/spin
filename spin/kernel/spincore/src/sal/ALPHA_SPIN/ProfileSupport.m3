...
TYPE
  ...
  ArcInfo = RECORD
    link   : [0 .. 16_FFFFFFFF] := 0;
    count  : [0 .. 16_FFFFFFFF] := 0;
    time   : Word.T := 0;
    selfPC : Word.T := 0;
  END;
  ...
  (* Keep this record synchronized with the structure in gprof.c *)
  CallerPC = RECORD
    proc   : [0 .. 16_FFFFFFFF] := 0;
    next   : [0 .. 16_FFFFFFFF] := 0;
    pc     : Word.T := 0;
  END;
...
PROCEDURE StoreSamples() =
  ...
  BEGIN
	...
	END;
      ELSE
	forGdb := NEW(UNTRACED REF ARRAY OF PCcount, PFM.GetBufferSize());
	numSamples := PFM.Read(forGdb^);
      END;
  ...
  BEGIN
    LOCK mu DO
      IF NOT callGraph THEN
        IO.Put("PC\t\t\tFrequency\n");
        IF forGdb # NIL THEN
          FOR i := FIRST(forGdb^) TO numSamples-1 DO
      ...
      ELSE
        ...
        FOR i := FIRST(gdbGraphProfile^) TO LAST(gdbGraphProfile^) DO
          ...
          IO.Put("CallerPC = " & 
            ...
            "\tCount = " & Fmt.Int(info.count) & 
            "\tCycles = " & Fmt.Int(info.time) & "\n");
        END;
      END;
    END;
  END PrintSamples;
