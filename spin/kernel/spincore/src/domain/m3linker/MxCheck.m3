 ...
 *	add Strip
 *      take out call to Clean from IsProgram
 *
 * 24-Jul-97  Tian Fung Lim (tian) at the University of Washington
 *	Uncommented out call to Clean.  This should be fixed (probably 
 *      by increasing traced heap size or by fixing webserver to 
 *      use appropriately-sized buffers)
 *
 * 11-Jun-97  Wilson Hsieh (whsieh) at the University of Washington
 *	IsProgram does not call Clean, so that the merge info is
 *        preserved
 *
 *)
...
PROCEDURE IsProgram (base: Mx.LinkSet;  errors : Wr.T): BOOLEAN =
  ...
  BEGIN
    ...
    (* should be deleted in future release, when memory pressure is better *)
    IF CleanNormal THEN Clean (s); END;
...
PROCEDURE Strip (base: Mx.LinkSet) =
  ...
  BEGIN
    (* should be deleted in future release, when memory pressure is better *)
    IF CleanNormal THEN RETURN; END;

    IF base = NIL THEN RETURN; END;

    RTIO.PutText ("In MxCheck.Strip\n");
    InitState (s, base, NIL);
    Clean (s);
  END Strip;
...
PROCEDURE CheckVirtualUnit (VAR s: State;  u: Mx.Unit) =
  BEGIN
    IF (u.mInfo.virtual) THEN
      s.import_err := NEW (ImportError, next := s.import_err, importer := u,
                           import_name := LAST(Mx.Name));
    END;
  END CheckVirtualUnit;

PROCEDURE PrintUnits (VAR s: State;  u: Mx.Unit) =
  BEGIN
    Wr.PutText(s.errors, M3ID.ToText(u.name));
    IF u.interface THEN 
      Wr.PutText(s.errors, ".i3 "); 
    ELSE 
      Wr.PutText(s.errors, ".m3 "); 
    END;
  END PrintUnits;

PROCEDURE CheckUnitImports (VAR s: State;  u: Mx.Unit) =
  BEGIN
    CheckUnitList (s, u, u.mInfo.imported_units);
    CheckUnitList (s, u, u.mInfo.exported_units);
    CheckUnitList (s, u, u.mInfo.used_interfaces);
...
PROCEDURE DumpMissingUnit (VAR s: State;  err: ImportError) =
  ...
  BEGIN
    ...
    WHILE (x # NIL) DO
      ...
      IF (u.mInfo.export_def_syms.cnt + u.mInfo.export_use_syms.cnt > 0) THEN
        Err (s, u_name, ": missing exported symbols:  ");
        DumpVSList (s, u, u.mInfo.export_def_syms);
        DumpVSList (s, u, u.mInfo.export_use_syms);
        ErrNL (s);
      END;
      r := u.revelations;
      IF (r # NIL) THEN
...
PROCEDURE DumpBadVS (VAR s: State;  u: Mx.Unit) =
  BEGIN
    IF   DumpBadVStamps (s, u, u.mInfo.export_def_syms, s.bad_vs)
      OR DumpBadVStamps (s, u, u.mInfo.export_use_syms, s.bad_vs)
      OR DumpBadVStamps (s, u, u.mInfo.import_def_syms, s.bad_vs)
      OR DumpBadVStamps (s, u, u.mInfo.import_use_syms, s.bad_vs) THEN
    END;
  END DumpBadVS;
...
PROCEDURE Clean (VAR s: State) =
  BEGIN
    ForEachUnit (s, CleanUnit);    
  END Clean;

PROCEDURE CleanUnit (<* UNUSED *> VAR s: State;  u: Mx.Unit) =
  BEGIN
    u.mInfo := NIL;
  END CleanUnit;
...
PROCEDURE ForEachUnit (VAR s: State;  p: UnitProc) =
  ...
  BEGIN
    ...
    FOR i := 0 TO LAST (x^) DO
      u := x[i].value;
      IF (u # NIL AND u.mInfo # NIL) THEN p (s, u) END;
    END;
