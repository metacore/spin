...
<* EXTERNAL *> PROCEDURE soqremque(
        so : T;
        q  : Ctypes.int): Ctypes.int;
...
<* EXTERNAL sosbunlock *> PROCEDURE sosbunlock(CVAR sb: SocketRep.sockbufT);

<* EXTERNAL soo_select *>
PROCEDURE soo_select(so: INTEGER;
		     which: [SalDep.FREAD .. SalDep.FWRITE];
		     curThread: REFANY): INTEGER;

<* EXTERNAL soaccept *> PROCEDURE soaccept(so:T; m:Mbuf.T): Errno.T;
