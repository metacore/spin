 ...
 *	We use Net.htonl rather than Swap to change byte order.
 *
 *)
    ...
    REPEAT
      ...
      ELSE
        ...
        REPEAT
          ...
          VIEW(rd.hdr, BITS 8 * HeaderBytes FOR 
                       ARRAY [1..HeaderBytes] OF CHAR) := 
              SUBARRAY(rd.buff^, rd.st, HeaderBytes);
          WITH swap = Net.nltoh(VIEW(rd.hdr.nb, 
                                     BITS 32 FOR Ctypes.unsigned_int)) DO
            rd.hdr.nb := VIEW(swap, Int32);
          END;
          IF rd.hdr.nb < 0 THEN 
            RAISE Rd.Failure(ProtocolErrorNB); 
          END;
...
PROCEDURE PutFrag(wr: WrT; eom: BOOLEAN) RAISES {Wr.Failure, Thread.Alerted} =
  VAR 
    len := wr.cur - wr.lo;
    hdr := FragmentHeader{eom := ORD(eom), nb := Net.htonl(len)};  
  BEGIN
    SUBARRAY(wr.buff^, 0, HeaderBytes) := VIEW(hdr, BITS 8 * HeaderBytes FOR
                                               ARRAY [1..HeaderBytes] OF CHAR);
