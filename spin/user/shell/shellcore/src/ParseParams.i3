...
TYPE
  ...
  Public = OBJECT 
    ...
    METHODS
      ...
      init (wr: Wr.T; argc: CARDINAL; argv: REF ARRAY OF TEXT): T;
        (* Allocates the arrays "arg" and "parsed" and
	   initializes them with the specified parameters.
	   Marks "arg[0]" as UNparsed, AND all others as unparsed, 
	   and sets "next" to 0.  Any subsequent parsing errors
	   will be printed out to "wr". *)
      
      keywordPresent (key: TEXT): BOOLEAN;
        (* Looks for the first unparsed argument "arg[i]"
	   that is equal to "key".  If so, returns "TRUE", marks
           arg[i] as parsed, and sets next to keyword following.
           Otherwise returns "FALSE" and 
           leaves self unchanged. *)

      FUNCTIONAL fkeywordPresent (key: TEXT): BOOLEAN;
        (* Looks for the first unparsed argument "arg[i]"
	   that is equal to "key".  If so, returns "TRUE",
      ...
      FUNCTIONAL ftestNext (key: TEXT): BOOLEAN;
        (* If "arg[next]" exists, is unparsed, and is equal to "key",
           returns TRUE.  Otherwise returns "FALSE". *)

      skipNext()  RAISES {Error};
        (*  INC(next) if arg[next] exists and is unparsed. Else, 
            raises ERROR. *)
      ...
      finish () RAISES {Error};
	   ...
	   prints a message and raises "Error". *)
      reset ();
        (* Return the parse object to its unparsed state *)
      dup () : T;
        (* Copy the unparsed keywords into a new T *)
   END;
