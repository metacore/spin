...
TYPE Byte = [0..255];
     Rec = RECORD first, second, third, fourth: Word.T; END;
...
PROCEDURE f(VAR x: Abyte; VAR a: Rec; VAR ar: ARRAY OF Forty8) =
  ...
  BEGIN
    TRY
      WITH y = VIEW(x, ARRAY [0..3] OF Word.T) DO
        INC(count);
        IF y[0] # 16_BEEF THEN wrong (0); END;
        IF y[1] # 16_FEED THEN wrong (1); END;
        IF y[2] # 16_FFFF THEN wrong (2); END;
        IF y[3] # 16_AAAA THEN wrong (3); END;
        IF VIEW(x, ARRAY[0..1] OF Word.T)[0] # 16_BEEF THEN
          wrong (4);
        END;
        IF VIEW(x, ARRAY[0..1] OF Word.T)[1] # 16_FEED THEN
          wrong (5);
        END;
      END;

      WITH y = VIEW(glob, ARRAY [0..3] OF Word.T) DO
        INC(count);
        IF y[0] # 16_BEEF THEN wrong (6); END;
        IF y[1] # 16_FEED THEN wrong (7); END;
        IF VIEW(glob, ARRAY [0..3] OF Word.T)[0] # 16_BEEF THEN
          wrong (8);
        END;
        IF VIEW(glob, ARRAY [0..3] OF Word.T)[1] # 16_FEED THEN
          wrong (9);
        END;
      END;
      ...
      WITH y = VIEW(x, Rec) DO
        INC(count);
        IF y.first # 16_BEEF THEN wrong (10); END;
        IF y.second # 16_FEED THEN wrong (11); END;
        IF VIEW(x, Rec).first # 16_BEEF THEN wrong (12); END;
        IF VIEW(x, Rec).second # 16_FEED THEN wrong (13); END;
      END;

      WITH y = VIEW(glob, Rec) DO
        INC(count);
        IF y.first # 16_BEEF THEN wrong (14); END;
        IF y.second # 16_FEED THEN wrong (15); END;
        IF VIEW(glob, Rec).first # 16_BEEF THEN wrong (16); END;
        IF VIEW(glob, Rec).second # 16_FEED THEN wrong (17); END;
      END;

      WITH y = VIEW(x, Word.T) DO
        INC(count);
        IF y # 16_BEEF THEN wrong (18); END;
        IF VIEW(x, Word.T) # 16_BEEF THEN wrong (19); END;
      END;

      WITH y = VIEW(glob, Rec2) DO
        INC(count);
        IF y.foo # 16_BEEF THEN wrong (20); END;
        IF VIEW(glob, Rec2).foo # 16_BEEF THEN wrong (21); END;
      END;

      WITH y = VIEW(x, Word.T) DO
        INC(count);
        IF y # 16_BEEF THEN wrong (22); END;
        IF VIEW(x, Word.T) # 16_BEEF THEN wrong (23); END;
      END;

      WITH y = VIEW(glob, Rec2) DO
        INC(count);
        IF y.foo # 16_BEEF THEN wrong (24); END;
        IF VIEW(glob, Rec2).foo # 16_BEEF THEN wrong (25); END;
        y.foo := 16_EAEAEAEA;
      END;
      ...
      WITH y = VIEW(a, ARRAY [0..1] OF Word.T) DO
        INC(count);
        IF y[0] # 16_EAEAEAEA THEN wrong (26); END;
        IF y[1] # 16_FEED THEN wrong (27); END;
        IF VIEW(a, ARRAY [0..1] OF Word.T)[0] # 16_EAEAEAEA THEN
          wrong (28);
        END;
        IF VIEW(a, ARRAY [0..1] OF Word.T)[1] # 16_FEED THEN
          wrong (29);
        END;
      END;
      ...
      WITH y = VIEW(a, Rec) DO
        INC(count);
        IF y.first # 16_EAEAEAEA THEN wrong (30); END;
        IF y.second # 16_FEED THEN wrong (31); END;
        IF VIEW(a, Rec).first # 16_EAEAEAEA THEN wrong (32); END;
        IF VIEW(a, Rec).second # 16_FEED THEN wrong (33); END;
      END;

      WITH y = VIEW(a, Word.T) DO
        INC(count);
        IF y # 16_EAEAEAEA THEN wrong (34); END;
        IF VIEW(a, Word.T) # 16_EAEAEAEA THEN wrong (35); END;
        y := 16_bbbbbbbb;
      END;
      ...
      WITH y = VIEW(x, ARRAY [0..3] OF Word.T) DO
        INC(count);
        IF y[0] # 16_BBBBBBBB THEN wrong (36); END;
        IF y[1] # 16_FEED THEN wrong (37); END;
        IF y[2] # 16_FFFF THEN wrong (38); END;
        IF y[3] # 16_AAAA THEN wrong (39); END;
      ...
      WITH y = VIEW(glob, ARRAY [0..1] OF Word.T) DO
        INC(count);
        IF y[0] # 16_BBBBBBBB THEN wrong(42); END;
        IF y[1] # 16_FEED THEN wrong(43); END;
        IF VIEW(glob, ARRAY [0..1] OF Word.T)[0] # 16_BBBBBBBB THEN
          wrong(44);
        END;
        IF VIEW(glob, ARRAY [0..1] OF Word.T)[1] # 16_FEED THEN
          wrong(45);
        END;
      END;
      ...
      WITH y = VIEW(x, Rec) DO
        ...
        IF y.second # 16_FEED THEN wrong(47); END;
        IF VIEW(x, Rec).first # 16_BBBBBBBB THEN wrong(48); END;
        IF VIEW(x, Rec).second # 16_FEED THEN wrong(49); END;
      END;

      WITH y = VIEW(glob, Rec) DO
        INC(count);
        IF y.first # 16_BBBBBBBB THEN wrong(50); END;
        IF y.second # 16_FEED THEN wrong(51); END;
        IF VIEW(glob, Rec).first # 16_BBBBBBBB THEN wrong(52); END;
        IF VIEW(glob, Rec).second # 16_FEED THEN wrong(53); END;
      ...
      WITH y = VIEW(x, Word.T) DO
        INC(count);
        IF y # 16_BBBBBBBB THEN wrong (54); END;
        IF VIEW(x, Word.T) # 16_BBBBBBBB THEN wrong (55); END;
        y := 16_adfadfad;
      END;

      WITH y = VIEW(glob, Word.T) DO
        INC(count);
        IF y # 16_ADFADFAD THEN wrong(56); END;
        IF VIEW(glob, Word.T) # 16_ADFADFAD THEN wrong(57); END;
      END;
      ...
      WITH y = VIEW(a, ARRAY [0..1] OF Word.T) DO
        INC(count);
        IF y[0] # 16_ADFADFAD THEN wrong(58); END;
        IF y[1] # 16_BADBAD THEN wrong(59); END;
        IF VIEW(a, ARRAY [0..1] OF Word.T) [0] # 16_ADFADFAD THEN
          wrong(60);
        END;
      ...
      WITH y = VIEW(a, Rec) DO
        INC(count);
        IF y.first # 16_ADFADFAD THEN wrong(62); END;
        IF y.second # 16_BADBAD THEN wrong(63); END;
        IF VIEW(a, Rec).first # 16_ADFADFAD THEN wrong(64); END;
        IF VIEW(a, Rec).second # 16_BADBAD THEN wrong(65); END;
      END;

      WITH y = VIEW(a, Word.T) DO
        INC(count);
        IF y # 16_ADFADFAD THEN wrong(66); END;
        IF VIEW(a, Word.T) # 16_ADFADFAD THEN wrong(67); END;
      END;
      ...
      WITH b = VIEW (a, ARRAY OF Word.T) DO
        INC(count);
        IF NUMBER (b) # 4 THEN wrong (208); END;
        IF b[0] # 16_ADFADFAD THEN wrong (209); END;
        IF VIEW (a, ARRAY OF Word.T)[0] # 16_ADFADFAD THEN
          wrong (210);
        END;
        IF NUMBER (VIEW (a, ARRAY OF Word.T)) # 4 THEN
          wrong (211);
        END;
        b[0] := 16_ADFADFDA;
        ...
        WITH c = VIEW (b, ARRAY OF Forty8) DO
          ...
          IF c[1].b # 16_FFFF THEN wrong (215); END;
          IF VIEW (b, ARRAY OF Forty8)[1].b # 16_FFFF THEN wrong (216); END;
    ...
    (* no alignment exception should occur on the PC *)
    TRY
    ...
    (* no alignment exception should occur on the PC *)
    TRY
      WITH y = VIEW(SUBARRAY (ar, 1, 1), ARRAY OF Word.T) DO<* NOWARN *>
      END;
    EXCEPT
      SpinException.Exception(info) =>
        wrong(305);
    ...
    IF VIEW(ar[FIRST (ar)], BITS 24 FOR [0..Word.Shift(1,24)-1]) # 16_FADFDA THEN
      wrong (407);
    END;

    IF VIEW(ar[LAST (ar)], Forty8).a # 16_00BA THEN
      wrong (408);
    END;
    IF VIEW(ar[LAST (ar)], Forty8).b # 16_FFFF THEN
      wrong (409);
    END;
    IF VIEW(ar[LAST (ar)], Forty8).c # 16_0 THEN
      wrong (410);
    END;
    ...
    IF VIEW(global2, RECORD a, b: ARRAY [0..3] OF Byte; c: REF Word.T; END).c #
      global2.third THEN
      wrong (500);
    END;
