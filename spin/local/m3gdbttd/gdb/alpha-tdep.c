...
/*
 * HISTORY
 * 29-Jun-96  Brian Bershad (bershad) at the University of Washington
 *	added dynamic pc_after_break evaluation.
 *
 * 03-May-96  Brian Bershad (bershad) at the University of Washington
 *	Fixed find_proc_desc to properly disambiguate between "unusual"
 *	 debugging information and the end of the line with dynamically
 *	 linked code; we were not making this distinction and anytime we
 *	 were seeing BLOCK_START(block_for_pc(pc)) > pc we figured we had
 *	 to have a trampoline_stub in our hand. This is not strictly
 *	 true; in cases where startaddress is zero AND BS(bfp(pc)) > pc
 *	 then we assume trampoline.
 *	 This code is really hairy.
 *
 */
...
#define XXDebug if (ttd_debug_enabled) printf
...
int
alpha_at_kernel_boundary PARAMS((FRAME));
     ...
     FRAME fci;
{
  ...
  if (fci->saved_regs)  {
	  printf("Warning: overriding fci->saved_regs for 0x%lx\n", fci);
  }
     ...
     FRAME frame;
{
  ...
  if (frame->next && frame->next->uk_frame) {
	  pcreg = PC_REGNUM;
  } else 
#endif
  {
	  pcreg = frame->signal_handler_caller ? PC_REGNUM
    ...
    FRAME next_frame;
{
    ...
    XXDebug("in heuristic proc desc 0x%lx\n", start_pc);
    ...
    FRAME next_frame;
{
  ...
  CORE_ADDR startaddr;
   ...
   XXDebug("in find proc_desc 0x%lx\n", pc);
  ...
  XXDebug("block_for_pc 0x%lx\n", b);  

  find_pc_partial_function (pc, NULL, &startaddr, NULL);
  XXDebug("startaddr 0x%lx\n", startaddr);        
  ...
  if (b == NULL)  {
    ...
    {
    XXDebug("start addr = %lx, BS = %lx\n",
			      startaddr, BLOCK_START(b));
    
      if (startaddr > BLOCK_START (b))  {
	/* This is the "pathological" case referred to in a comment in
	   print_frame_info.  It might be better to move this check into
	   symbol reading.  */
	XXDebug("setting sym to NULL\n");	  
	sym = NULL;
      }
      else  {
	sym = lookup_symbol (MIPS_EFI_SYMBOL_NAME, b, LABEL_NAMESPACE,
			     0, NULL);
	XXDebug("setting sym to 0x%lx\n", sym);
      }
     }
  ...
  if (sym)
	...
	XXDebug("sym ! NULL; lookup heuristic for proc_desc 0x%lx\n", proc_desc);	
	if (next_frame == NULL)
	  {
...
#ifdef RUNTIME_GENERATED_CODE
    if (startaddr == 0)  {
      CORE_ADDR pc;
      struct symbol *sym;
      alpha_extra_func_info_t proc_desc;

      XXDebug("Watch out, in RUNTIME_GEN_CODE\n");
  
      sym = lookup_symbol("trampoline_stub",NULL, VAR_NAMESPACE, NULL, NULL);
      if(sym != NULL) {
	pc = BLOCK_START (SYMBOL_BLOCK_VALUE (sym));
	proc_desc = find_proc_desc(pc, next_frame);
	if (proc_desc != NULL)  {
	    XXDebug("Gack. Returning proc_desc for trampoline code\n");
	  return proc_desc;
	}
      }
    }
...
void
alpha_print_extra_pc_function_info(pc, s)
    CORE_ADDR pc;
    struct symtab s;
{
    alpha_extra_func_info_t proc_desc = find_proc_desc(pc, 0);
    if (!proc_desc) {
	printf_filtered("no alpha specific information!\n");
	return;
    }

    if (proc_desc->pdr.framereg < NUM_REGS)
      printf_filtered (" frame pointer is at %s+%d\n", 
                       reg_names[proc_desc->pdr.framereg], 
                                 proc_desc->pdr.frameoffset);
}
    ...
    FRAME frame;
{
	...
	char *name;
        if (frame->uk_frame) {
		/* If we're at a kernel entry point... */
	        return frame->frame;
	}
	...
	if (IN_SIGTRAMP (saved_pc, name))
	  ...
	  return 0;
   } else return read_next_frame_reg(frame, PROC_FRAME_REG(proc_desc))
     ...
     struct frame_info *fci;
{
  ...
  if (proc_desc)
    ...
    if (fci->uk_frame) {
	    ...
	    printf("Duping regs from uk frame to top frame \n");
	    *fci->saved_regs = *fci->next->saved_regs;
    }
}
...
/*
 * For debugging remote kernels, we always get the right pc. Otherwise.
 * we have to adjust by 4.
 */
int
alpha_decr_pc_after_break()
{
	if (strncmp(current_target->to_shortname, "ttd", 3) == 0) 
		return 0 ;
	else return 4;
}
...
/*
 * Support code for remote kernel debugging.
 * Remote machine dependent code for the ALPHA architecture for GDB.
 * Code in here must deal with the fact that ttd state may not always be
 * conformant to the calling conventions of a single programming language.
 * In particular, when debugging across the user kernel boundary, we must
 * translate frames from the target's exception frame format to gdb format.
 */


static char *kernel_boundary_functions[] = {
	"exception_no_server",
	"mach_msg_continue",
...
#define NUM_BOUNDARY_FUNCTIONS (sizeof(kernel_boundary_functions) / sizeof(kernel_boundary_functions[0]))

static int
alpha_in_trap_vector(FRAME frame)
{
	alpha_extra_func_info_t proc_desc;
	int tv = FALSE;
	char *name;
	...
	if (frame->proc_desc) 
		...
		frame->proc_desc = proc_desc = find_proc_desc(frame->pc, frame->next);

	if (!frame->proc_desc) return FALSE;

	find_pc_partial_function(proc_desc->pdr.adr, &name, NULL, NULL);
        if (ttd_debug_enabled)
	      printf("Checking if frame for %s is in trap_vector\n", name);

#ifdef notdef	
	for (i = 0; i < NUM_BOUNDARY_FUNCTIONS; i++)  {
		if (strcmp(name, kernel_boundary_functions[i]) == 0) {
			tv = TRUE;
			break;
		}
	}
#else
	if (strncmp(name, "TRAP_", 5) == 0)  {
		tv = TRUE;
	}
#endif
	
	if (!tv)  {
		/* Check the hard way. Look for a stack frame that looks hokey */
		if (proc_desc == NULL)  {
			error("in_trap_vector: no proc desc for queried frame\n");
		}
		tv = (PROC_PC_REG(proc_desc) == ZERO_REGNUM) && (PROC_REG_MASK(proc_desc) & IM_EXC) &&
			(PROC_FRAME_OFFSET(proc_desc) == sizeof(struct ttd_alpha_saved_state));
	}
	if (tv) {
  	        if (ttd_debug_enabled)		
		   printf("Found a vector function 0x%lx \n", frame);
	}
	return tv;
}

int
alpha_at_kernel_boundary(FRAME frame)
{
	if (frame->uk_frame) return FALSE;	
	return alpha_in_trap_vector(frame);
}
	
int
alpha_find_saved_user_registers(FRAME frame)
{
#ifdef READ_USER_REGISTER_ADDRESSES
        if (ttd_debug_enabled)	
		printf("Reading user registers addresses for frame 0x%lx)\n", frame);
	if (!frame->saved_regs) {
	...
	READ_USER_REGISTER_ADDRESSES(&frame->saved_regs->regs[0]);

	
	frame->saved_regs->regs[SP_REGNUM] = read_memory_integer(
				 frame->saved_regs->regs[SP_REGNUM],
				 8);
	
	frame->frame = 	frame->saved_regs->regs[SP_REGNUM];

	if (ttd_debug_enabled)
		printf("alpha_find_saved_user: fixing up fci for 0x%lx to point to sp(0x%lx), vm(0x%lx)\n",
	       frame,
	       frame->saved_regs->regs[SP_REGNUM],
	       frame->saved_regs->regs[FP_REGNUM]);	
...
FRAME
alpha_make_kernel_boundary_frame(FRAME prev)
{
	FRAME boundary;

	if (prev->prev) {
		error("alpha_make_kernel_boundary_frame: frame already exists\n");
	}
	boundary = create_new_frame(0, 0);
	if (alpha_find_saved_user_registers(boundary)) {
		if (!prev->saved_regs) {
			alpha_find_saved_regs(prev);
		}
		boundary->pc = read_memory_integer(boundary->saved_regs->regs[PC_REGNUM], 8);

		boundary->next = prev;
		boundary->prev = prev->prev;
		if (boundary->prev)
			boundary->prev->next = boundary;
		boundary->uk_frame = 1;
/* 		prev->uk_frame = 1; */
		prev->prev = boundary;
/*		prev->frame = (CORE_ADDR)0;*/
		prev = boundary;
/*		boundary->frame = 0;*/
	}
	return prev;
