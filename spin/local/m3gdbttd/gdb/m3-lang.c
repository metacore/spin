...
/*
 * HISTORY
 * 02-Jan-97  Wilson Hsieh (whsieh) at the University of Washington
 *	fixed so that PROCANY type uid is recognized correctly
 *
 * 14-Aug-96  Frederick Gray (fgray) at the University of Washington
 *	Changed printf to fprintf in a debugging message; it was causing 
 *      control characters to spew everywhere on the x86.
 *
 * 20-Jul-96  Wilson Hsieh (whsieh) at the University of Washington
 *	support for ALIGNED FOR types
 *
 * 28-Jun-96  Przemek Pardyak (pardy) at the University of Washington
 *	Encode typecode in 20 bits as in SRC distribution.
 *
 * 07-Apr-96  Brian Bershad (bershad) at the University of Washington
 *	Made runtime conditional "nomangle" to ensure that MI, MM and
 *	 INITM symbols are visible when needed. (not sure still when this
 *	 is!)
 *
 * s16-becker
 * 	Added SELECTIVE_MANGLE so MI_, MM_, and _INITM_ symbols are
 *	in the visible symbol table.
 *
 * 23-Jan-96  Brian Bershad (bershad) at the University of Washington
 *	Support for new typecode representation for SPIN if target
 *	 supports it. Separate out OS dependent m3 services.
 *
 */
...
struct m3_os_ops *m3_ops = (struct m3_os_ops*)0;
...
{
    if (m3_ops) {
	fprintf_filtered(stdout, "Target m3 initialized as %s\n",
			 m3_ops->os_name);
	return;
    }
    

    m3_ops = init_target_os_m3_ops_posix();
    if (!m3_ops)
	m3_ops = init_target_os_m3_ops_spin();

    if (m3_ops) {
	fprintf_filtered(stdout, "Target m3 initialized as %s\n",
			 m3_ops->os_name);
    } else {
	fprintf_filtered(stdout, "Could not initialize thread target m3\n");
    }
    return;
     ...
     int from_tty;
{
  ...
  m3_ops->os_first_m3_thread (&first);

  /* scan for a match */
  cur = first;
  while ((cur.bits) && (cur.id != target_id)) {
    m3_ops->os_next_m3_thread (&cur);
  }
...
{
    if (m3_ops) {
	m3_ops->os_threads(args, from_tty);
    } else {
	error("Target thread implementation not known\n");
    }
}
     ...
     struct type *t;
{
  ...
  switch (key [1])
    ...
    case 'E':
      TYPE_CODE (t) = TYPE_CODE_M3_ALIGNED;
      sscanf (type_specific_info, "%ld", &TYPE_M3_ALIGNED_ALIGN (t));
      set_field_uid (t, 0);
      break;
     ...
     char *uid;
{
  ...
  if (m3_debug_enabled) {
	printf("m3_resolve_type: %s -> %s\n",
	uid, (sym ?  SYMBOL_NAME(sym) : "nil"));
  }
  ...
  error ("Cannot resolve type with uid %s %x", uid, uid_val);
}
     ...
     char *name;
{
  char struct_name [100];
  if (nomangle()) {
	  sprintf (struct_name, "M%c_%s", kind, name);
...
init_m3_constants ()
{
  ...
  /* SPIN */
  /* Pardy changed this code to be different from when we are dealing with
   * POSIX code.  It works with spin code, but not clear why we need to
   * be different. A pain in the butt that we have to have two arms here to
   * get at rt0u__types__value
   */
  rt0u__types = lookup_symbol ("RT0u__types", 0, VAR_NAMESPACE, 0, 0);

  /* XX */
  if (rt0u__types == NULL)  {
      /* maybe we're on a posix box?? */
      struct symbol *rt0u;
      rt0u = find_m3_ir('I', "RT0u");
      if (find_m3_rec_field(SYMBOL_TYPE(rt0u), "types",
			&rt0u_types_size, &rt0u_types_offset, 0))  {
      /* from spin-23 m3gdbttd */	  
	      target_read_memory (SYMBOL_VALUE_ADDRESS (rt0u) +
				  rt0u_types_offset / 8,
				(char *)&rt0u_types_value, 
				rt0u_types_size / 8);
      } else
	    m3_error("could not find RT0u.types");	  
  } else {
      target_read_memory (SYMBOL_VALUE_ADDRESS (rt0u__types),
			(char *)&rt0u_types_value, 
			8);	/* XX should not be 8 */
  }
}
     ...
     CORE_ADDR addr;
{
  ...
  /* the typecode is in Modula-3 bits 4..24 */
#if TARGET_BYTE_ORDER == BIG_ENDIAN
  /* BUG BUG BUG: THE FOLLOWING ASSUMES THAT THE WORD SIZE IS 32 BITS !!! */
  /* BUG BUG BUG: DEC SRC DISTRIBUTION CONTAINS THE SAME BUG              */
  typecode = (typecode >> 8) & 0xfffff;
#else
  typecode = (typecode >> 4) & 0xfffff;
#endif
     ...
     register LONGEST *upper;
{
  ...
  tc = TYPE_CODE (type);
  while (tc == TYPE_CODE_M3_PACKED || tc == TYPE_CODE_M3_ALIGNED) {
    /* this works because packed and aligned have the same format */
    type = TYPE_M3_PACKED_TARGET (type);
    tc = TYPE_CODE (type);
  }
...
nomangle()
{
    return 0 && ttd_in_get_all_domain_info();	/* XX */
}

    

void
m3_reinitialize(args, from_tty)
	char *args;
	int from_tty;
{
	if (from_tty)
		printf_filtered("Reinitializing typecode information\n");
	rt0u_types_value = 0;	/* force reinitialization */
	init_m3_constants();
	init_thread_constants();
}
...
void
_initialize_m3()		/* start in column 0 */
{
	add_cmd("m3-reinitialize",
		class_files,
		m3_reinitialize,
		"Reconstruct Modula-3 type information.",
		&cmdlist);


	add_show_from_set
		(add_set_cmd ("m3-debug", class_support, var_boolean,
			      (char *)&m3_debug_enabled,
