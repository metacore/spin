...
/* SPIN mod
	If there is no .text section (lower_sect==0), then try for .data.
	It looks the the if 0 code above would do this automatcially.
	I do not know what its commented out and do know the section
	name I want in this case.  So we will look for .data explicitly.

	gdb also would print a warning if lowest is not .text, but that
	is common for .io files, so that is commented out too.
*/

      if (lowest_sect == 0)
      	lowest_sect = bfd_get_section_by_name (objfile->obfd, ".data");
      if (lowest_sect == 0)
