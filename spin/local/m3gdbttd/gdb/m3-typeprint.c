 ...
 *	Some not-very satisfactory support for printing recursive
 *	 types.  However you slice it, it's ugly.
 *	 
 *	
 *
 */
...
void
m3_type_print_base1 PARAMS ((struct type *, FILE *, int, int));
...
static struct type *shown[256];


/*
 * Record potentially recursive types
 */
static int
m3_mark_type(struct type *type)
{
    int i;
    if (TYPE_CODE(type) == TYPE_CODE_M3_OBJECT ||
	TYPE_CODE(type) == TYPE_CODE_M3_RECORD) {
	    for (i = 0; i < 256; i++)  {
		if (!shown[i]) {
		    shown[i] = type;
		    return 1;
		} else if (shown[i] == type)
		    return 0;
            }
            fprintf_filtered(stdout,"Warning. Recursive type overflow\n");
            return 0;
    } else
	return 1;
}
...
static void
m3_unmark_type(struct type *type)
{
    int i;
    if (TYPE_CODE(type) == TYPE_CODE_M3_OBJECT ||
	TYPE_CODE(type) == TYPE_CODE_M3_RECORD) {
		...
		if (shown[i] == type)
            ...
            fprintf_filtered(stdout,"Warning. Lost a type\n");
    }
}
...
m3_type_print_base (type, stream, show, level)
     ...
     int level;
{
    int showtype = m3_mark_type(type);
    m3_type_print_base1(type, stream, showtype? show : 0, level);
    m3_unmark_type(type);
}


void
m3_type_print_base1 (type, stream, show, level)
     struct type *type;
     FILE *stream;
     int show;
     int level;
{
  ...
  switch (TYPE_CODE (type))
    ...
    case TYPE_CODE_M3_ALIGNED:
      fprintf_filtered (stream, "ALIGNED %d FOR ",
			TYPE_M3_ALIGNED_ALIGN (type));
      m3_type_print_base (TYPE_M3_TARGET (type), stream, show-1, level);
      break;
