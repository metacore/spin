 ...
 *      Check for null dest buffer and zero len xfer requests. Fail in
 *       first case, succeed trivially in second.
 */
...
int
target_function_is_ignored(fn)
	int (*fn)();
{
	return (fn == (int (*)())ignore);
}
...
#define de_fault(field, value) \
  ...
  de_fault (to_notice_signals,		(int (*)())ignore);
  de_fault (to_pid_to_str,		normal_pid_to_str);
  de_fault (to_next,			0);
     ...
     int write;
{
  ...
  /* simple checks to keep us from doing boneheaded accesses. */
  if (myaddr == NULL) return -1;
  if (len == 0) return 0;
