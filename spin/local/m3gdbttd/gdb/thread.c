     ...
     int pid;
{
	...
	struct thread_info *newtp;  
  

	newtp = (struct thread_info *) xmalloc (sizeof (struct thread_info));


	/* simple case. front of list */
	if (!thread_list || pid < thread_list->pid)  {
		newtp->next = thread_list;
		thread_list = newtp;
	} else {
     ...
     int from_tty;
{
  ...
  extern int target_function_is_ignored();
  ...
  for (tp = thread_list; tp; tp = tp->next)
  {
      if (target_has_execution) {
	      if (!target_function_is_ignored(current_target->to_notice_signals))  {
		      if ( (*current_target->to_notice_signals)(tp->pid) == -1) {
			      tp->pid = -1; /* DEAD */
			      continue;
		      }
	      } else if( kill (tp->pid, 0) == -1)     {
...
struct cmd_list_element *thread_apply_list = NULL;
