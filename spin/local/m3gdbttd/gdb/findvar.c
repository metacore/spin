     ...
     int regnum;
{
...
#ifdef FRAME_AT_USER_KERNEL_BOUNDARY
  /* The registers for a user/kernel frame (and the faked one on top of it)
   * are stored in the frame itself,
   * not on the frames of functions that the function calls.
   */
  if (frame->uk_frame) {
	/*  printf("find_saved_register: returning reg %d\n", regnum);*/
  ...
  while (1)
	...
	break;
      fi = get_frame_info (frame1);			/* XX (check new version)*/
      get_frame_saved_regs (fi, &saved_regs);
