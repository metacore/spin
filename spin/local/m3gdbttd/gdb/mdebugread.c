     ...
     struct section_offsets *section_offsets;
{
  ...
  switch (sh->sc)
    ...
    case scText:
	 ...
	 corresponding start symbol value, do not relocate it.  */
      /* SPIN: There was a bug where scText/stBlock symbols were
	 relocated here, but later on, at the other spot marked SPIN,
	 the value was expected to be a displacement from the procedure
	 start. The fix is to add the test sh->st != stBlock right here.*/
  ...
  switch (sh->st)
      ...
      if (sh->st == stProc)
	{
	 ...
	 table. */
      memcpy (lookup_function_type (t), SET_SYMBOL_TYPE (s), sizeof (struct type));
#else
    ...
    case_stBlock_code:
	 ...
	 is the displacement from procedure start */
      /* SPIN: There was a bug here where the symbol value used
	 to be relocated for scText/stBlock symbols. See the
	 previous SPIN comment for the fix */
      push_parse_stack ();
