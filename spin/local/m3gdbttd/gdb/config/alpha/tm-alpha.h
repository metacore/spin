...
#define BAD_REGNUM 66		/* Bad address */
#define CAUSE_REGNUM		/* Cause */
...
extern void alpha_print_extra_pc_function_info PARAMS ((CORE_ADDR , struct symtab *));

#define PRINT_EXTRA_PC_FUNCTION_INFO(pc, s)  \
...
/*
 * this is used to determine if we are looking at a frame at the
 * user kernel boundary. We should really be inside tm-mach3 or tm-spin
 */
#define FRAME_AT_USER_KERNEL_BOUNDARY(fi) \
	alpha_at_kernel_boundary(fi)

extern struct frame_info *alpha_make_kernel_boundary_frame
					PARAMS(( struct frame_info*));

#define FRAME_PREV_USER_KERNEL_FRAME(frame) \
	(frame) = alpha_make_kernel_boundary_frame(frame)

/*
 * this is used to determine if we are looking at a frame that
 * was generated at run-time.  The debugger might not know anything
 * about the symbol but the target may, so go ask the target.
 */

#define RUNTIME_GENERATED_CODE

#include "ttd_alpha-mach3_types.h"
#include "ttd_alpha-mach3.h"
