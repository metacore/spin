...
PROCEDURE WriteFlags (VAR s: State;  flags: REF ARRAY OF TEXT) =
  BEGIN
    MxIO.PutTxt (s.buf, Target.EOL, "F");
    IF flags # NIL THEN
      MxIO.PutInt (s.buf, NUMBER(flags^), " ");
      VAR
        c := " ";
      BEGIN
        FOR i := FIRST(flags^) TO LAST(flags^) DO
          IF i = LAST(flags^) THEN c := Target.EOL; END;
          MxIO.PutTxt (s.buf, flags[i], c);
        END;
      END;
    ELSE
      MxIO.PutInt (s.buf, 0, Target.EOL);
    END;
