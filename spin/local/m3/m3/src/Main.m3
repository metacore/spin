 ...
 *	Compile flags added to link information to guarantee that imported
 *	interfaces are recompiled using the same flags.  The only flag
 *	used now is "-SpinRelax".  The flags could be also used
 *	by the dynamic linker to verify the correctness of code, e.g.,
 ...
 *	added -RefCount and -NoFloat options
 *
 * 28-Jan-97  Marc Fiuczynski (mef) at the University of Washington
 *	Added linkinfo_ok option to turn of the generation of dynamic
 *	linking information.  By default the m3 compiler still generates
 *	the dynamic link info, which is roughly 512-1500 bytes of ascii
 *	per .io and .mo file. To turn it off use m3_option("-NoLinkInfo").
 *
 * 03-Jan-97  Charles Garrett (garrett) at the University of Washington
 *	Added an Arg.List called asm_opts. These are options that go to
 *	 pass 1 of the compiler (the CC program) and are only intended
 *	 for assembly language files, not for C files. We need this to
 *	 prevent the profiling of assembly language files.
 *
 ...
 *	Added support for embedding interface information to object files.
 *
 * 22-Feb-96  Charles Garrett (garrett) at the University of Washington
 *	Added GenConfFile option which prints the object files to a file
 *	 in a good initialization order.
 *
 ...
 *
 * 29-Dec-95  Przemek Pardyak (pardy) at the University of Washington
 *	Whisted.  Previous SPIN changes: overridable name of the entry
 *	procedure, "-genmain", "-M", and "-SpinRelax" options.
 *
 *)
...
VAR
  ...
  (* Options which are only used for assembly language and not for C code *)
  cc_opts       : Arg.List  := Arg.NewList ();
  as_opts       : Arg.List  := Arg.NewList ();
  link_files    : TEXT      := "-z2@@";
  include_dir   : TEXT      := NIL;
  link_coverage : TEXT      := NIL;
  link_flags    : Arg.List  := NIL;
...
VAR
  ...
  mainname   : TEXT := "main";		              (* SPIN *)
  genmain    : BOOLEAN := FALSE;		      (* SPIN *)

VAR
  magic     := NEW (IntRefTbl.Default).init (100); (* type name -> info *)
  ast_cache := NEW (IntRefTbl.Default).init (100); (* intf name -> AST *)
...
(** RTCollector.Enable (); **)
    ...
    IF (compile_failed) THEN Process.Exit (1) END;
  END DoIt;
...
PROCEDURE ParseCommandLine () =
  ...
  BEGIN
    Msg.Out("Compiler invoked as: ", Params.Get(0), "\n");
...
PROCEDURE ParseOption (arg: TEXT;  arg_len: INTEGER;  rest: Arg.List) =
  ...
  BEGIN
    ...
    | 'a' => IF (arg_len = 2) THEN
             ...
             ELSIF Text.Equal(arg, "-as") THEN
               Arg.Append(as_opts, GetArg(arg, rest));  ok := TRUE;
             END;
    ...
    | 'c' => IF (arg_len = 2) THEN
             ...
             ELSIF Text.Equal(arg, "-cc") THEN
               Arg.Append(cc_opts, GetArg(arg, rest));  ok := TRUE;
    ...
    | 'g' => IF (arg_len = 2) THEN
             ...
             ELSIF Text.Equal(arg, "-gui") THEN
               gui := TRUE;
             ELSIF Text.Equal (arg, "-genmain") THEN	(* SPIN *)
               genmain := TRUE;				(* SPIN *)
             ELSE
               do_debug := TRUE;
    ...
    | 'G' => IF Text.Equal(arg, "-GenConfFile") THEN
               confFile := GetArg(arg, rest);
               ok := TRUE;
             END;
    ...
    (* SPIN *)
    | 'M' => IF (arg_len = 2)	
               THEN mainname := GetArg (arg, rest); ok := TRUE;
               ELSE mainname := Text.Sub (arg, 2, arg_len); ok := TRUE;
             END;
    ...
    (* SPIN *)
    | 'N' => IF Text.Equal(arg, "-NoLoopholeWarning") THEN
               Arg.Append (pass_0.args, "-NoLoopholeWarning"); ok := TRUE;
             ELSIF Text.Equal(arg, "-NoFloat") THEN
               Arg.Append (pass_0.args, "-NoFloat"); ok := TRUE;
               Arg.Append (link_flags, "-NoFloat");
             ELSIF Text.Equal(arg, "-NoLinkInfo") THEN
               linkinfo_ok := FALSE;
               MxGen.linkinfo_ok := FALSE;
               Arg.Append (pass_0.args, "-NoLinkInfo"); ok := TRUE;
             ELSIF Text.Equal(arg, "-NoProcedureTypeInfo") THEN
               Arg.Append (pass_0.args, "-NoProcedureTypeInfo"); ok := TRUE;
             END;
    ...
    | 'R' => IF Text.Equal (arg, "-RefCount") THEN
               Arg.Append (pass_0.args, "-RefCount"); ok := TRUE;
               Arg.Append (link_flags, "-RefCount");
             ELSIF Text.Equal (arg, "-ReadBarrier") THEN
               Arg.Append (pass_0.args, "-ReadBarrier"); ok := TRUE;
               Arg.Append (link_flags, "-ReadBarrier");
             END;
    ...
    (* SPIN *)
    | 'S' => IF Text.Equal(arg, "-SpinRelax") THEN
               Arg.Append (pass_0.args, "-SpinRelax");  ok := TRUE;
               Arg.Append (link_flags, "-SpinRelax");
             END;

    | 'T' => IF Text.Equal (arg, "-TraceRef") THEN
               Arg.Append (pass_0.args, "-TraceRef"); ok := TRUE;
               Arg.Append (link_flags, "-TraceRef");
             ELSE
               INC (n_tfiles);
               IF (arg_len = 2)
                THEN Unit.ResetPath ();  n_tfiles := 0;
    ...
    (* SPIN *)
    | 'V' => IF Text.Equal(arg, "-VerboseChecks") THEN
               Arg.Append (pass_0.args, "-VerboseChecks");  ok := TRUE;
             END;

    | 'W' => IF Text.Equal (arg, "-WriteBarrier") THEN
               Arg.Append (pass_0.args, "-WriteBarrier"); ok := TRUE;
               Arg.Append (link_flags, "-WriteBarrier");
             END;
...
PROCEDURE DumpConfiguration () =
  ...
  BEGIN
    ...
    Msg.OutL ("pass 1 [cc]  += ", NIL, cc_opts);
    Msg.OutL ("pass 1 [as]  += ", NIL, as_opts);
...
PROCEDURE MatchLocalUnit (u: Mx.Unit) =
...
PROCEDURE DumpLinkInfo () =
  ...
  BEGIN
    ...
    MxOut.WriteUnits (units, wr, Arg.Flatten(link_flags, NIL));
  ...
  BEGIN
    WHILE (a # NIL) DO
      ...
      ELSE
        Msg.Debug ("adding units: ");
        WHILE (ux # NIL) DO
          AddLibraryUnit (ux.unit, a.library);
          ux := ux.next;
        END;
        Msg.Debug (Wr.EOL);
      END;
...
PROCEDURE AddLibraryUnit (u: Mx.Unit;  lib: TEXT) =
...
PROCEDURE CompileS (f: FileInfo) =
  BEGIN
    ...
    ELSIF (f.name.kind = NK.S) THEN
      Pass1 (f.source, f.object, f.name.base, as_opts);
  ...
  BEGIN
    ...
    ELSIF (f.name.kind = NK.C) THEN
      IF (bootstrap_mode)
        ...
        ELSE Pass1 (f.source, f.object, f.name.base, cc_opts);
      END;
      Utils.NoteNewFile (f.object);
    ELSIF (pass_6.cmd = NIL) THEN
  ...
  BEGIN
    IF (NOT make_mode) THEN
      IF (f.name.kind = NK.I3) OR (f.name.kind = NK.M3)
        ...
        ELSE Msg.Explain (" -> compiling ", f.source);
      END;
      RETURN TRUE
    END;
    ...
    *********************************************************)
    ...
    IF (objTime = Utils.NO_TIME)
      ...
      IF (f.name.kind = NK.I3) OR (f.name.kind = NK.M3)
        THEN f.stale_src := TRUE; (* defer the message for a moment *)
        ELSE Msg.Explain ("new source -> compiling ", f.source);
      END;
      ETimer.Pop ();
      RETURN TRUE;
    END;
...
TYPE
  ...
  OVERRIDES
    ...
    get_link_info_text := Pass0_GetLinkInfoText;
  END;

VAR (* CONST *)
  env: Env := NEW (Env);

PROCEDURE ResetEnv (f: FileInfo; source, object: TEXT) =
  BEGIN
    env.file                  := f;
    env.source                := source;
    env.object                := object;
  ...
  BEGIN
    ...
    IF (ok) AND ((f.stale_src) OR (f.missing_info)) THEN
      ...
      ELSIF (NOT make_mode) THEN
        f.stale_src := FALSE;
        Msg.Explain (" -> compiling ", f.source);
      ELSE
        f.stale_src := FALSE;
        Msg.Explain ("new source -> compiling ", f.source);
      END;
    END;

    (* do the compilation *)
    IF (ok) THEN
      ResetEnv (f, f.source, object);
...
PROCEDURE Pass0_CheckImports (f: FileInfo; VAR source: M3Compiler.SourceFile) =
  VAR ids: M3Compiler.IDList;  ref: REFANY;
  BEGIN
    ResetEnv (f, source.name, NIL);
...
PROCEDURE Pass0_Open (<*UNUSED*> env: Env;  name: M3ID.T;
  ...
  BEGIN
    file := Unit.Open (M3ID.ToText (name), interface, generic);

    (* if this is an interface for which link info alrady exists then
       probably it is pulled in from a library; use the compiler
       flags used to generate the link info *)
    VAR
      ref: REFANY;
      f: FileInfo;
    BEGIN
...
PROCEDURE Pass0_GetLinkInfoText (env: Env): TEXT =
  VAR
    f: FileInfo := env.file;
    units: Mx.UnitList; 
  BEGIN
    IF env.unit # NIL THEN
      f.link_info := FinishUnitInfo (env);
    END;
    units := NEW (Mx.UnitList, unit := f.link_info, next := NIL);
    RETURN MxOut.UnitsToText(units, Arg.Flatten(link_flags, NIL));
  END Pass0_GetLinkInfoText;


(*------------------------------------------------ compilations and links ---*)

PROCEDURE Pass1 (source, object, base: TEXT; opts : Arg.List := NIL) =
  ...
  BEGIN
    ...
    (* Add any special arguments specified by the caller of Pass1 *)
    IF opts # NIL THEN
      VAR
        opt := opts.head;
      BEGIN
        WHILE opt # NIL DO
          Arg.Append(args, opt.arg);
          opt := opt.next;
        END;
      END;
    END;
...
PROCEDURE BuildProgram () =
  ...
  BEGIN
    ...
    IF NOT MxCheck.IsProgram (link_base, Stdio.stdout) THEN
      IF has_loader THEN Utils.Remove (Desc_file); END;
    ...
    MxGen.GenerateMain (link_base, wr,
		        mainname,	(* SPIN *)
			Msg.level >= Msg.Level.Debug,
    ...
    ELSE
      ...
      Pass1 (Main_C, Main_O, "_m3main", cc_opts);
      IF (compile_failed) THEN
...
PROCEDURE BuildLibrary () =
  ...
  BEGIN
    ...
    ETimer.Push (M3Timers.chkpgm);
    IF NOT MxCheck.IsLibrary (link_base, Stdio.stdout) THEN
    ...
    IF genmain THEN
      (* check for an up-to-date Main_O *)
      time_O := Utils.ModificationTime (Main_O);
      time_C := Utils.ModificationTime (Main_C);
      IF (time_O < time_C) OR (time_C = Utils.NO_TIME) THEN
        (* we must compile the linker generated code *)
        init_code := Main_C;
      ELSE
      ...
      Msg.Commands ("generate ", init_code);
      wr := Utils.OpenWriter (init_code, fatal := TRUE);
      MxGen.GenerateMain(link_base,wr, mainname, Msg.level >= Msg.Level.Debug,
	                        gui AND (target_os = M3Path.OSKind.Win32));
      Utils.CloseWriter (wr, init_code);
      ETimer.Pop ();
      ...
      ELSE
        IF (init_code = Main_XX) THEN
          ...
          Utils.Remove (Main_XX);
        END;
        Msg.Debug ("compiling ", Main_C, " ...\n");
        Pass1 (Main_C, Main_O, "main", cc_opts);
        IF (compile_failed) THEN
          Msg.FatalError (NIL, "cc ", Main_C, " failed!!");
        END;
      END;
    ...
    (* create a conf file for a spindle *)
    IF confFile # NIL THEN
      wr := Utils.OpenWriter (confFile, fatal := TRUE);
      MxGen.GenerateConf(link_base, wr,
                         Msg.level >= Msg.Level.Debug,
                         gui AND (target_os = M3Path.OSKind.Win32));
      Utils.CloseWriter (wr, confFile);
    END;
    ...
    IF (pass_4.cmd # NIL) THEN      ETimer.Push (M3Timers.pass_4);
      args := Arg.NewList ();
...
PROCEDURE FName (f: FileInfo): TEXT =
  BEGIN
    IF (f.source # NIL) AND (f.library # NIL) THEN
      RETURN f.source & " in library " & f.library;
    ELSIF (f.source # NIL) THEN
      RETURN f.source;
