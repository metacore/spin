...
PROCEDURE BitwiseAnd (READONLY a, b: Int;  VAR r: Int): BOOLEAN =
  (* It is safe for r to alias a or b *)
  BEGIN
    FOR i := 0 TO last_chunk DO
      r.x [i] := And (a.x[i], b.x[i]);
    END;
    RETURN TRUE;
  END BitwiseAnd;

PROCEDURE BitwiseOr (READONLY a, b: Int;  VAR r: Int): BOOLEAN =
  (* It is safe for r to alias a or b *)
  BEGIN
    FOR i := 0 TO last_chunk DO
      r.x [i] := Or (a.x[i], b.x[i]);
    END;
    RETURN TRUE;
  END BitwiseOr;

PROCEDURE Shift (READONLY a, b: Int;  VAR r: Int): BOOLEAN =
  (* It is safe for r to alias a or b *)
  VAR 
    shift : INTEGER;
    tmp, carry: INTEGER;
  BEGIN
    IF ToInt(b, shift) THEN
      (* Do the whole chunk shifts first. *)
      IF shift >= 0 THEN
        WITH chunks = shift DIV ChunkSize DO
          FOR i := last_chunk TO chunks BY -1 DO
            r.x[i] := a.x[i - chunks];
          END;
          
          FOR i := chunks-1 TO 0 BY -1 DO
            r.x[i] := 16_0000;
          END;
        END;

        WITH rest = shift MOD ChunkSize DO
          IF rest > 0 THEN
            carry := 0;
            FOR i := 0 TO last_chunk DO
              tmp := Word.Or(Word.LeftShift(r.x[i], rest), carry);
              carry := Word.RightShift(Word.And(tmp, 16_ffff0000), 16);
              r.x[i] := Word.And(tmp, 16_ffff);
            END;
          END;
        END;
      ...
      ELSE
        WITH chunks = -shift DIV ChunkSize DO
          FOR i := 0 TO last_chunk - chunks DO
            r.x[i] := a.x[i + chunks];
          END;
            
          FOR i := last_chunk - chunks + 1 TO last_chunk DO
            r.x[i] := 16_0000;
          END;
        END;

        WITH rest = -shift MOD ChunkSize,
             bitmask = Word.RightShift(16_ffff, ChunkSize - rest) DO
          IF rest > 0 THEN
            carry := 0;
            FOR i := last_chunk TO 0 BY -1 DO
              tmp := Word.And(r.x[i], bitmask);
              r.x[i] := Word.Or(Word.RightShift(r.x[i], rest), carry);
              carry := Word.LeftShift(tmp, ChunkSize - rest);
            END;
          END;
        END;
      END;           
