...
(*                                                             *)
(* Last modified on Tue Dec 20 14:06:06 PST 1994 by kalsow     *)
 ...
 *	Disabled a test that clashed with new code generation for NARROW.
 *
 *)

MODULE M3CG_Check;

IMPORT Wr, Thread, Fmt, Text, Stdio, IntRefTbl;
...
<* UNUSED *> PROCEDURE IsOK (need, got, prev: ST): BOOLEAN =
  CONST
...
<* UNUSED *> PROCEDURE ST_Name (a, prev: ST): TEXT =
  BEGIN
    IF (a = ST.Match) THEN a := prev END;
    RETURN ST_name [a];
  END ST_Name;

PROCEDURE Stack_Pop (self: U;  <* UNUSED *> a: ST; b, c, d: ST) =
  <* UNUSED *>VAR
    s0 := Stack_Get (self, 0);
    s1 := Stack_Get (self, 1);
    s2 := Stack_Get (self, 2);
  ...
  BEGIN
(*
  FIXME: DISABLED THIS TEST TO BE ABLE TO COMPILE NEW NARROW CODE
...
PROCEDURE get_runtime_hook (self: U;  n: Name; VAR v: Var; VAR o: ByteOffset) =
  VAR ref: REFANY;
  BEGIN
...
PROCEDURE check_size (self: U;  READONLY i: Target.Int) =
  (* IF (s0.I < i) THEN Abort; Pop (1) *)
  BEGIN
    self.s_pop (ST.Int);
    self.child.check_size (i);
  END check_size;

PROCEDURE check_align (self: U;  READONLY i: Target.Int) =
  (* IF (s0.I BITAND i) THEN Abort; Pop (1) *)
  BEGIN
    self.s_pop (ST.Addr);
    self.child.check_align (i);
  END check_align;
...
PROCEDURE pop_param (self: U;  t: ZType) =
