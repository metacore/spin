 ...
 *	functional
 *
 * 23-Mar-96  Wilson Hsieh (whsieh) at the University of Washington
 *	missed 2 places where ArrayType.Split was called
 *
...
IMPORT Int, EnumType, IntegerExpr, EnumExpr, TypeExpr;
...
PROCEDURE TypeOf (ce: CallExpr.T): Type.T =
  ...
  BEGIN
    ...
    t := Expr.TypeOf (e);
    index := NIL;
    IF ArrayType.Split (Type.Base(t), index, element) THEN
      IF (index = NIL) THEN index := Int.T END;
    ELSIF TypeExpr.Split (e, t) THEN
      IF NOT ArrayType.Split (Type.Base(t), index, element) THEN index := t END;
    END;
    IF (index = NIL) THEN index := Int.T; END;
