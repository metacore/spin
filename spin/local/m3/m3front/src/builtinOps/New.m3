...
(* File: New.m3                                                *)
(* Last Modified On Fri Feb 24 07:47:40 PST 1995 By kalsow     *)
(*      Modified On Fri Jan 25 08:10:52 1991 By muller         *)

(*
 * HISTORY
 * 06-Feb-97  Wilson Hsieh (whsieh) at the University of Washington
 *	field initialization must be ref counted
 *
 * 24-Mar-96  Wilson Hsieh (whsieh) at the University of Washington
 *	fixed bug from Modula-3 web page
 *	NEW(NULL, REFANY, or ADDRESS) were allowed, but are illegal
 *
 *)
...
PROCEDURE CheckRef (r: Type.T;  ce: CallExpr.T;  VAR cs: Expr.CheckState) =
  ...
  BEGIN
    IF (r = NIL) THEN
      Error.Msg("cannot NEW a variable of type REFANY, ADDRESS, NULL, or PROCANY");
      RETURN;
    END;
...
PROCEDURE CheckOpaque (t: Type.T; ce: CallExpr.T;  VAR cs: Expr.CheckState): Type.T =
  (* we already know that t is not an object, so we only need to
     check for a full revelation that says it is a REF *)
...
PROCEDURE GenRecord (t, r: Type.T;  traced: BOOLEAN;
  ...
  BEGIN
    ...
    FOR i := 1 TO LAST (ce.args^) DO
      ...
      IF traced THEN
        AssignStmt.Emit (field.type, value, M3.Global.Yes);
      ELSE
