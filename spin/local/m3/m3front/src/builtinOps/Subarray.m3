 ...
 *	add some comments to generated code
 *
 * 20-May-97  Charles Garrett (garrett) at the University of Washington
 *	Added low priority warnings for the cases where run-time checks
 *	are emitted.
 *
 ...
 *	added bug fix to SUBARRAY so that compiler does not crash
 *	from Modula-3 Web page listing bug fixes
 *
 * 25-Oct-95  Wilson Hsieh (whsieh) at the University of Washington
 *	make built-in operations non-bounded
 *
 * 21-Oct-95  Wilson Hsieh (whsieh) at the University of Washington
...
PROCEDURE Prep (ce: CallExpr.T) =
  ...
  BEGIN
    ...
    VAR
      pelts, pmax: INTEGER;
    BEGIN
      EVAL TInt.ToInt(n_elts, pelts);
      EVAL TInt.ToInt(max, pmax);
      
      CASE case OF
      | 0 =>  (* fixed array, var start, var len --------------------*)

        CG.Comment (-1, "Subarray: case 0\n");
        ...
        ELSE
          IF Host.verbose_checks THEN
            Error.Warn (1, "Emitting SUBARRAY check high against " & 
              Fmt.Int(pelts));
          END;
      ...
      | 4 =>  (* open array, var start, var len -------------------------------*)
        ...
        IF Host.doRangeChk THEN
          IF Host.verbose_checks THEN
            Error.Warn (1, "Emitting SUBARRAY check high against open array size\n");
          END;
