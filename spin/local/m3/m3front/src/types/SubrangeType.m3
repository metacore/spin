...
(* Last modified on Wed Sep  7 15:34:35 PDT 1994 by kalsow     *)
 ...
 *	Fixed to test for types Target.CGType.Int and Target.CGType.Word 
 *      in SetRep.
 *
 * 24-Jul-96  Wilson Hsieh (whsieh) at the University of Washington
 *	fix BitWidth so that 2^63-1 gets handled correctly
 *
 * 05-Jan-96  Wilson Hsieh (whsieh) at the University of Washington
 *	added ToText, which overrides the to_text inherited 
 *	method added name argument to New, name field.
 *
 * 13-Nov-95 Przemek Pardyak (pardy) at the University of Washington
 *	Fixed two bugs: changed BitWidth to use "<" instead of "<="
 *	(2^n requires n+1 bits of representation), changed the comparison
 *	used in SetRep to be dependent on the type of the value being compared
 *	(either Word or Int), integer comparison was used before for all
 *	values which would turn maximum value of Word into -1
 *)
...
IMPORT Target, TInt, TWord, TargetMap, Fmt;
...
PROCEDURE New (READONLY min, max: Target.Int;
               base: Type.T;  builtin: BOOLEAN; name: TEXT := NIL): Type.T =
  VAR p: P;
  BEGIN
    p := NEW (P);
...
PROCEDURE SetRep (p: P) =
  ...
  BEGIN
    ...
    wordOK := NOT (TInt.LT(p.min, TInt.Zero) OR TInt.LT(p.max, TInt.Zero));

    FOR i := FIRST (TargetMap.Int_types) TO LAST (TargetMap.Int_types) DO
      CASE TargetMap.Int_types[i].cg_type OF
      | Target.CGType.Int_A,
        Target.CGType.Int_B, 
        Target.CGType.Int_C,
        Target.CGType.Int_D,
        Target.CGType.Int => 
        IF TInt.LE (TargetMap.Int_types[i].min, p.min) AND 
          TInt.LE (p.max, TargetMap.Int_types[i].max) 
         THEN
          p.rep := i; 
          RETURN;
        END;
      | Target.CGType.Word_A,
        Target.CGType.Word_B, 
        Target.CGType.Word_C,
        Target.CGType.Word_D,
        Target.CGType.Word => 
        IF wordOK AND
          TWord.LE (TargetMap.Int_types[i].min, p.min) AND
          TWord.LE (p.max, TargetMap.Int_types[i].max)
         THEN
          p.rep := i; 
          RETURN;
        END;
      ELSE
        Error.Msg ("INTERNAL ERROR: range is not an integer or word");
        <* ASSERT FALSE *>
      END;
    END;
    p.rep := LAST (Rep);
...
PROCEDURE BitWidth (n: Target.Int;  VAR width: INTEGER;  VAR neg: BOOLEAN) =
  ...
  BEGIN
    ...
    FOR i := 0 TO LAST (power) DO
      IF TInt.LT (n, power[i]) THEN width := i; EXIT; END;

      (* special case; one less bit is necessary for
         the largest positive INTEGER *)
      IF TInt.EQ (power[i], Target.Integer.max) AND
         TInt.LE (n, Target.Integer.max) THEN
        width := i; EXIT;
      END;
    END;
  END BitWidth;
...
PROCEDURE ToText (p: P): TEXT =
  ...
  BEGIN
    IF (p.name # NIL) THEN
      RETURN (p.name);
    ELSE
      EVAL TInt.ToInt(p.min, lo);
      EVAL TInt.ToInt(p.max, hi);
      RETURN ("[" & Fmt.Int(lo) & ".." & Fmt.Int(hi) & "]");
    END;
  END ToText;
