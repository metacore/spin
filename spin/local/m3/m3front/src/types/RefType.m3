...
(* File: RefType.m3                                            *)
(* Last changed: bershad                                       *)

(* Last modified on Tue Dec 20 14:37:26 PST 1994 by kalsow     *)
 ...
 *
 * 08-Jul-96  Charles Garrett (garrett) at the University of Washington
 *	Fix a bug with PROCANY fingerprints. Compiler used to assume that
 *	 PROCANY's had a referent type which they don't really have.
 *
 * 16-Apr-96  Wilson Hsieh (whsieh) at the University of Washington
 *	removed conflict lines from whist entries
 *	fixed some irrelevantly changed code to match pristine code
 *
 * 15-Feb-96  Charles Garrett (garrett) at the University of Washington
 *	ToText now returns UNTRACED REF for untraced types. See PR/79.
 *
 * 22-Feb-96  Przemek Pardyak (pardy) at the University of Washington
 *	Modified InitTypecell to always generate a name for an object
 *	type and to prepend it with a unit name.
 *
 * 05-Jan-96  Wilson Hsieh (whsieh) at the University of Washington
 *	added ToText, which overrides the to_text inherited method
 *      added name argument to New for holding "REFANY" or "ADDRESS",
 *      and name field to P
 *
 *)
...
IMPORT ProcType, ObjectAdr, Word, M3RT, Procany;

IMPORT Host, Procedure, Runtime;  (* for reference counting *)
...
PROCEDURE Check (p: P) =
  ...
  BEGIN
    ...
    IF (NOT p.isTraced) AND (info.isTraced) THEN
      IF Module.IsSafe() THEN
        Error.Msg ("unsafe: untraced ref type to a traced type");
      ELSE
        Error.Warn (2, "untraced ref type to a traced type");
      END;
    END;
...
PROCEDURE InitTypecell (t: Type.T;  offset, prev: INTEGER) =
  ...
  BEGIN
    ...
    VAR
      full_name := ToText(p);
    BEGIN
      IF M3ID.ToText(Module.Name(NIL)) # NIL THEN
        p.user_name := M3ID.ToText(Module.Name(NIL));
      ELSE
        Error.Msg("internal error: cannot find the current unit");
      END;
      IF Module.IsInterface () THEN
        p.user_name := p.user_name & ".i3: "
      ELSE
        p.user_name := p.user_name & ".m3: "
      END;
      p.user_name := p.user_name & full_name;
    END;
...
PROCEDURE GenRC (t: Type.T; definitelyGlobal: BOOLEAN;
  ...
  BEGIN
    ...
    IF definitelyGlobal THEN
      IF Host.trace_ref THEN
        proc := Runtime.LookUpProc (Runtime.Hook.TraceRef);
      ELSE
        proc := Runtime.LookUpProc (Runtime.Hook.AssignTracedToGlobal);
      END;
    ELSE (* maybe global *)
      IF Host.trace_ref THEN
        proc := Runtime.LookUpProc (Runtime.Hook.TraceRefPossible);
      ELSE
        proc := Runtime.LookUpProc (Runtime.Hook.AssignTracedToPossibleGlobal);
      END;
    END;
    CG.Comment (-1, "Assignment of traced pointer to global\n");
    ...
    CG.Comment (-1, "End assignment of traced pointer to global\n");
...
PROCEDURE FPrinter (p: P;  VAR x: M3.FPInfo) =
  BEGIN
    ...
    ELSIF Type.IsEqual (p, Null.T, NIL) THEN
      x.tag := "$null";
      x.n_nodes := 0;
    ELSIF Type.IsEqual (p, Procany.T, NIL) THEN
      x.tag := "$procany";
      x.n_nodes := 0;
    ELSE
...
PROCEDURE ToText (p: P): TEXT =
  BEGIN
    IF (p.user_name # NIL) THEN
      RETURN (p.user_name);
    ELSIF (p.name # NIL) THEN
      RETURN (p.name);
    ELSE
      IF p.isTraced THEN
        RETURN ("REF " & p.target.to_text());
      ELSE
        RETURN ("UNTRACED REF " & p.target.to_text());
      END;
    END;
  END ToText;
