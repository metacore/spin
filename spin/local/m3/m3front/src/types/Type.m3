...
(*                                                             *)
(* File: Type.m3                                               *)
(* Last Modified On Tue Dec 20 15:27:29 PST 1994 by kalsow     *)
 ...
 *	fix IsAssignable for procedures in unsafe modules
 *        can only assign from PROCANY, not from elsewhere within
 *        the type hierarchy
 *
 * 19-Jun-96  Wilson Hsieh (whsieh) at the University of Washington
 *	moved VIEW-related code to ViewCheck.m3
 *
 * 16-Apr-96  Wilson Hsieh (whsieh) at the University of Washington
 *	cannot implicitly NARROW a PROCANY in a safe module
 *      cannot implicitly NARROW a PROCEDURE in a safe module
 *
 * 14-Apr-96  Marc Fiuczynski (mef) at the University of Washington
 *	Changed VIEW to allow the viewing of SETs.
 *
 ...
 *      added procedure Noop which is the default for the set_align
 *       method in Type.T
 *
 * 03-Mar-96  Wilson Hsieh (whsieh) at the University of Washington
 *	have to deal with Class.Aligned
 *
 * 05-Feb-96  Wilson Hsieh (whsieh) at the University of Washington
 *	fixed count of number of bits for check for
 *	 representation-completeness
 *	2 errors: variable that was set to Target.Word_D.size was not
 *	 being initialized
 *	enums and subranges get packed automatically by the compiler
 *
 * 09-Jan-96  Wilson Hsieh (whsieh) at the University of Washington
 *	fixed bug in ToText, which crashed if arg was NIL
 *
 * 08-Jan-96  Wilson Hsieh (whsieh) at the University of Washington
 *	made CanView, CanViewAs more precise when dealing with
 *	enumerated types, subranges, and packed types
 *
 * 04-Jan-96  Wilson Hsieh (whsieh) at the University of Washington
 *	added NullText, which is the default to_text Type method
 *
...
IMPORT AlignedType, Procany, ViewCheck;
...
PROCEDURE Parse (): T =
  ...
  BEGIN
    ...
    | TK.tALIGNED   => t := AlignedType.Parse ();
    ...
    | TK.tPROCEDURE,
      TK.tBOUNDED,
      TK.tFUNCTIONAL=> t := ProcType.Parse ();
...
PROCEDURE Noop (<* UNUSED *> t: T; <* UNUSED *> new_align: INTEGER) =
  BEGIN
  END Noop;
...
PROCEDURE StripPacked (t: T): T =
  ...
  BEGIN
    ...
    LOOP
      ...
      ELSIF (u.info.class = Class.Aligned)THEN u := AlignedType.Base (u);
      ELSE  RETURN u;
      END;
      IF    (v.info.class = Class.Named)  THEN v := NamedType.Strip (v);
      ELSIF (v.info.class = Class.Packed) THEN v := PackedType.Base (v);
      ELSIF (v.info.class = Class.Aligned)THEN v := AlignedType.Base (v);
      ELSE  RETURN v;
      END;
      IF    (v.info.class = Class.Named)  THEN v := NamedType.Strip (v);
      ELSIF (v.info.class = Class.Packed) THEN v := PackedType.Base (v);
...
PROCEDURE IsStructured (t: T): BOOLEAN =
  BEGIN
    ...
    | Class.Packed, Class.Aligned =>
        RETURN IsStructured (Base (t));
    | Class.Record,
      Class.Array,
...
PROCEDURE SetAlign (t: T; new_align: INTEGER) =
  BEGIN
    IF (t = NIL) THEN RETURN; END;
    t.set_align (new_align);
  END SetAlign;
  ...
  BEGIN
    ...
    | Class.Integer, Class.Real, Class.Longreal, Class.Extended,
      ...
      Class.Ref, Class.Subrange =>
        CG.Load_indirect (t.info.cg_type, 0, t.info.size);
    | Class.Packed =>
    ...
    | Class.Error, Class.Named, Class.Array, Class.OpenArray, Class.Record =>
        (* skip -- either it is structured, or it is an error *)
    END;
  END LoadScalar;
...
PROCEDURE IsOrdinal (t: T): BOOLEAN =
  ...
  BEGIN
    RETURN (c = Class.Integer) OR (c = Class.Subrange)
           ...
           OR ((c = Class.Packed OR c = Class.Aligned) AND
                IsOrdinal (StripPacked (t)));
  END IsOrdinal;
...
PROCEDURE Number (t: T): Target.Int =
  ...
  BEGIN
    ...
    ELSIF (c = Class.Error) THEN
      RETURN TInt.Zero;
    ELSIF (c = Class.Packed OR c = Class.Aligned) THEN
...
PROCEDURE IsEquiv (a, b: T;  x: Assumption; canTruncate: BOOLEAN): BOOLEAN =
  ...
  BEGIN
    ...
    IF RepresentationComplete (b) THEN
      IF canTruncate THEN
        IF a.info.size = -1 OR b.info.size = -1 THEN
          RETURN RepresentationComplete (a);
        ELSE
          RETURN a.info.size >= b.info.size AND
                 RepresentationCompleteFor (a, b.info.size);
        END;
      ELSE
        (* since truncation is only allowed at top-level,
           we should not run into an open array *)
        <* ASSERT a.info.size # -1 AND b.info.size # -1 *>
        RETURN a.info.size = b.info.size AND RepresentationComplete (a);
      END;
    END;

    IF (ac = bc) THEN
      RETURN a.isEquiv (b, y, canTruncate);
    ELSE
      RETURN FALSE;
    END;
  END IsEquiv;
...
PROCEDURE IsSubtype (a, b: T): BOOLEAN =
  ...
  BEGIN
    ...
    IF (bc = Class.Named) THEN b := Strip (b);  bc := b.info.class END;
    IF (ac = Class.Error) OR (bc = Class.Error) THEN RETURN TRUE END;
    IF (ac = Class.Packed) OR (ac = Class.Aligned) THEN
      a := StripPacked (a)
    END;
    IF (bc = Class.Packed) OR (bc = Class.Aligned) THEN
      b := StripPacked (b)
    END;
...
PROCEDURE IsAssignable (a, b: T): BOOLEAN =
  ...
  BEGIN
    ...
    ELSIF IsSubtype (a, b) THEN
      ...
      RETURN IsSubtype (b, Reff.T)             (* NARROW *)
          OR ArrayType.Split (b, i, e)

          (* narrow of a non-traced type *)
          OR (IsSubtype (b, Addr.T)

              (* SPIN: we made procedures subtypes of ADDRESS, but
                 we do not allow procedure types to be narrowed *)
              AND NOT ProcType.Is (b)
              AND
                  (NOT Module.IsSafe ()
                    OR
                   (* cannot narrow from generic types in safe modules *)
                   (NOT IsEqual (b, Addr.T, NIL)
                     AND
                    NOT IsEqual (b, Procany.T, NIL))
                  ));
    ELSE
      RETURN FALSE;
    END;
...
PROCEDURE InitCompilation () =
  BEGIN
    ...
    (* once Procany is in the runtime the following line can be uncommented *)
(*    EVAL TypeTbl.Put (compiled, Check(Procany.T),   Procany.T);*)
...
PROCEDURE Zero (full_t: T) =
  ...
  BEGIN
    ...
    | Class.Integer, Class.Subrange, Class.Enum =>
        ...
        CG.Store_indirect (u.info.cg_type(*CG.Type.Int*), 0, size);
    | Class.Real =>
...
PROCEDURE GenRC (t: T; definitelyGlobal: BOOLEAN; noOverlap := FALSE) =
  BEGIN
    IF t = NIL THEN RETURN; END;
    t.genRC (definitelyGlobal, noOverlap);
  END GenRC;

PROCEDURE RCDefault (t: TT;
                     <* UNUSED *> definitelyGlobal: BOOLEAN;
                     <* UNUSED *> noOverlap := FALSE) =
  BEGIN
    CG.Copy (t.info.size, overlap := FALSE);
  END RCDefault;
...
PROCEDURE NeverEquiv (a, b: TT; <*UNUSED*> x: Assumption;
...
PROCEDURE NullText (<* UNUSED *> t: TT): TEXT =
  BEGIN
    RETURN ("");
  END NullText;
