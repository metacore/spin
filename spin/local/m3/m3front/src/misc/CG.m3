...
(*                                                             *)
(* File: CG.m3                                                 *)
(* Last modified on Tue Dec 20 14:18:39 PST 1994 by kalsow     *)
(*      modified on Tue May 25 11:19:53 PDT 1993 by muller     *)

(*
 * HISTORY
 * 27-May-97  Wilson Hsieh (whsieh) at the University of Washington
 *	improved error message in FindIntType
 *
 ...
 *	Got rid of Convert.
 *
 * 06-Jun-96  Charles Garrett (garrett) at the University of Washington
 *	SRC released a bug fix to the Set_range procedure. This procedure
 *	 used to fill in the wrong bits of a SET.
 *
 * 05-Feb-96  Wilson Hsieh (whsieh) at the University of Washington
 *	added check_align operation for VIEW alignment check
 *
 * 13-Nov-95  Przemek Pardyak (pardy) at the University of Washington
 *	Add Convert procedure to bypass a limit in code generator.
 *
...
PROCEDURE Declare_aligned (t: TypeUID; size, align: Size; base: TypeUID) =
  BEGIN
    cg.declare_aligned (t, size, align, base);
    (* need to implement the following *)
    (* WebInfo.Declare_aligned (t, base); *)
  END Declare_aligned;
...
PROCEDURE Get_runtime_hook (n: Name;  VAR v: Var;  VAR o: Offset) =
  BEGIN
...
PROCEDURE Pop (): Val =
  ...
  BEGIN
    ...
    (* make sure it is not bound to the M3CG stack *)
  ...
  BEGIN
    WITH x = stack [SCheck (1, "Force")] DO
      ...
      (** x.align     := TargetMap.CG_Align [x.type];
        --- we are not changing the alignment of this value **)
    END;
  END Force;
...
PROCEDURE Copy_n (s: Size;  overlap: BOOLEAN) =
  ...
  BEGIN
    ...
    (* convert the count into a multiple of a machine type size *)
...
PROCEDURE Check_size (READONLY i: Target.Int) =
  BEGIN
    EVAL Runtime.LookUpProc (Runtime.Hook.SizeFault);
    Force ();
    cg.check_size (i);
    SPop (1, "Check_size");
  END Check_size;

PROCEDURE Check_align (READONLY i: Target.Int) =
  BEGIN
    EVAL Runtime.LookUpProc (Runtime.Hook.AlignFault);
    Force ();
    cg.check_align (i);
    SPop (1, "Check_align");
  END Check_align;
...
PROCEDURE FindIntType (t: Type;  s: Size;  o: Offset;  a: Alignment): MType =
  ...
  BEGIN
    ...
    Err ("Unaligned integer: size " & Fmt.Int (s + o MOD a) &
         " bits, alignment " & Fmt.Int (a) & " bits.");
    RETURN t;
  END FindIntType;
