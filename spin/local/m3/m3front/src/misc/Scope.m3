...
(*
 * HISTORY
 * 01-Jun-96  Wilson Hsieh (whsieh) at the University of Washington
 *      support for functional: InFunctional
 *
 * 22-Feb-96  Charles Garrett (garrett) at the University of Washington
 *	Checks the compilation phase before printing some warnings, so
 *	 that duplicate warnings are suppressed. See PR/42.
 *
 * 25-Oct-95  Wilson Hsieh (whsieh) at the University of Washington
 *	support for bounded: InBounded
 *
 *)
...
IMPORT Scanner, ValueRep, Variable, Tracer, Convert, Host;
...
REVEAL
  M3.Scope = M3.Node BRANDED "Scope.T" OBJECT
    ...
    in_bounded : BOOLEAN  := FALSE;
    in_functional : BOOLEAN  := FALSE;
  END;
...
PROCEDURE PushNew (open: BOOLEAN;  name: M3ID.T;  module := FALSE;
                   nested := FALSE;
                   inBounded := FALSE; inFunctional := FALSE): T =
  VAR t := NEW (T);
  BEGIN
    ...
    (* procedure scopes must have one enclosing scope *)
    IF (top # NIL) THEN
      t.in_bounded := top.in_bounded OR inBounded;
      t.in_functional := top.in_functional OR inFunctional;
    END;
...
PROCEDURE LookUp (t: T;  name: M3ID.T;  strict: BOOLEAN): Value.T =
  ...
  BEGIN
    ...
    IF (Scanner.in_main AND NOT Host.currentPhase = Host.Phase.EmittingCode) THEN
...
PROCEDURE InBounded (t: T): BOOLEAN =
  BEGIN
    RETURN t.in_bounded;
  END InBounded;

PROCEDURE InFunctional (t: T): BOOLEAN =
  BEGIN
    RETURN t.in_functional;
  END InFunctional;
