(*
 * HISTORY
 * 27-Sep-96  Yasushi Saito (yasushi) at the University of Washington
 *	Changed to ignore pragmas used in sieg.
 *)
...
(* Create the list of ignored pragmas. They are used in Sieg. *)
CONST IgnoredPragmas = ARRAY OF TEXT
  {
   "AS", "INTERFACE_PROC_BASE", "OUT", "SYNONYMS", "PROCID",
   "EPILOG_BRANCH_ON_REGISTER", "CTEXT", "INTERNAL",
   "DESCENDING_PROC_IDS"
   };
						
PROCEDURE DefaultIgnoreList (): IDList =
  VAR l: IDList := NIL;
  BEGIN
    FOR i := FIRST(IgnoredPragmas) TO LAST(IgnoredPragmas) DO
      l := NEW(IDList, id := M3ID.Add(IgnoredPragmas[i]), next := l);
    END;
    RETURN l;
  END DefaultIgnoreList;
