...
PROCEDURE Compile (p: P) =
  VAR
    pmin, pmax: INTEGER;
  BEGIN
    EVAL TInt.ToInt(p.min, pmin);
    EVAL TInt.ToInt(p.max, pmax);
    ...
    | Class.cLOWER => 
      IF Host.verbose_checks THEN
        Error.Warn (1, "Emitting explicit check low against " & 
          Fmt.Int(pmin));
      END;
      CG.Check_lo (p.min);
    | Class.cUPPER => 
      IF Host.verbose_checks THEN
        Error.Warn (1, "Emitting explicit check high against " & 
          Fmt.Int(pmax));
      END;
      CG.Check_hi (p.max);
    | Class.cBOTH  => 
      IF Host.verbose_checks THEN
        Error.Warn (1, "Emitting explicit check range in [" & 
          Fmt.Int(pmin) & ", " & Fmt.Int(pmax) & "]");
      END;
      CG.Check_range (p.min, p.max);
    END;
  END Compile;
  ...
  BEGIN
    ...
    IF Host.doRangeChk THEN
      ...
      IF TInt.LT (minE, min) AND TInt.LT (max, maxE) THEN
        IF Host.verbose_checks THEN
          Error.Warn (1, "Emitting check range for expression of type " &
            Type.ToText(Expr.TypeOf(e)) & " in [" & Fmt.Int(pmin) & ", " & 
            Fmt.Int(pmax) & "]");
        END;
      ...
      ELSIF TInt.LT (minE, min) THEN
        ...
        END;
        IF Host.verbose_checks THEN
          Error.Warn (1, "Emitting check low for expression of type " &
            Type.ToText(Expr.TypeOf(e)) & " against " & Fmt.Int(pmin));
        END;
        CG.Check_lo (min);
      ELSIF TInt.LT (max, maxE) THEN
        ...
        END;
        IF Host.verbose_checks THEN
          Error.Warn (1, "Emitting check high for expression of type " &
            Type.ToText(Expr.TypeOf(e)) & " against " & Fmt.Int(pmax));
        END;
        CG.Check_hi (max);
      END;
    END;
