 ...
 *	Fixed compile time NARROW failure by adding line marked PR/20 below.
 *
 *)
...
PROCEDURE CompileLV (p: P) =
  ...
  BEGIN
    ...
    | Class.cMODULE =>
      CASE Value.ClassOf (p.obj) OF
      | Value.Class.Expr => Value.Load (p.obj);       (* Fixes PR/20 *)
...
PROCEDURE IsExplicitMethod (e: Expr.T; VAR t: Type.T): BOOLEAN =
  BEGIN
    ...
    | NULL => RETURN FALSE;
    | P(p) =>
      IF p.class = Class.cOBJTYPE THEN
        t := p.objType;
        RETURN TRUE;
      ELSE
        RETURN FALSE;
    ...
    ELSE
      RETURN FALSE;
    END;
  END IsExplicitMethod;

PROCEDURE IsGlobal (p: P) : M3.Global =
  BEGIN
    IF p.automagic OR p.class = Class.cOBJFIELD THEN
