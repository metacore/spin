...
(* File: ArrayExpr.m3                                          *)
(* Last modified on Fri Feb 24 16:40:25 PST 1995 by kalsow     *)
(*      modified on Tue Mar 12 00:29:44 1991 by muller         *)

(*
 * HISTORY
 * 26-Feb-97  Wilson Hsieh (whsieh) at the University of Washington
 *	call ref counting code for open array constructors
 *      write barrier code
 *
...
PROCEDURE DoOpen (p: P;  elt_pack, elt_align: INTEGER) =
  ...
  BEGIN
    ...
    EVAL Type.CheckInfo (element, element_info);
    CG.Push (t2);
    AssignStmt.Emit (element, p.args[0], M3.Global.Yes, compiled := TRUE);
    ...
    FOR i := 1 TO n_args-1 DO
      ...
      Expr.Prep (p.args[i]);
      CG.Load_addr (t1);
      AssignStmt.Emit (element, p.args[i], M3.Global.Yes);
    END;
