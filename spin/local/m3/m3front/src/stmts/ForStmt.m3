 ...
 *	Variable.Split return a BOOLEAN now
 *
 *)
...
PROCEDURE Compile (p: P): Stmt.Outcomes =
  VAR
    ...
    offset: INTEGER;
  BEGIN
    EVAL Variable.Split (p.var, type, indirect, readonly);
    ...
    (* or if its body can fall through.  -- Ernst Heinz *)
    IF (from = NIL) OR (limit = NIL) OR (step = NIL) OR
