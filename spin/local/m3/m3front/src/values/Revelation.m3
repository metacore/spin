...
PROCEDURE Parse (READONLY att: Decl.Attributes) =
  ...
  BEGIN
    ...
    END;
    IF att.isBounded    THEN Error.Msg ("a revelation cannot be EPHEMERAL"); END;
    IF att.isFunctional THEN Error.Msg ("a revelation cannot be FUNCTIONAL"); END;
    IF att.isImplicit THEN Error.Msg ("a revelation cannot be IMPLICIT"); END;
