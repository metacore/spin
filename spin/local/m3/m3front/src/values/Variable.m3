...
(* Last Modified Bershad				       *)
(* Last Modified On Wed Mar  1 08:53:15 PST 1995 By kalsow     *)
(*      Modified On Thu Dec  5 17:21:40 PST 1991 By muller     *)

(*
 * HISTORY
 * 27-Feb-97  Wilson Hsieh (whsieh) at the University of Washington
 *	call ref counting code for open arrays passed by value
 *      call ref counting code for user-specified initializations
 *      write barrier code
 *
 * 10-Oct-96  Wilson Hsieh (whsieh) at the University of Washington
 *	remove warning about IO being imported and not used
 *	
 * 19-Sep-96  Wilson Hsieh (whsieh) at the University of Washington
 *	add by_reference field for allowing "globalness" to be checked by
 *	 IsGlobal
 *
 * 29-Jul-96  Charles Garrett (garrett) at the University of Washington
 *	Changed so that WITH temporaries pointing to structured types are
 *	 not thought to be structured themselves.
 *
 * 28-May-96  Wilson Hsieh (whsieh) at the University of Washington
 *	add IsGlobal
 *	disallow functional, bounded, implicit
...
PROCEDURE ParseDecl (READONLY att: Decl.Attributes) =
  ...
  BEGIN
    ...
    IF att.isBounded    THEN Error.Msg ("a variable cannot be EPHEMERAL"); END;
    IF att.isFunctional THEN Error.Msg ("a variable cannot be FUNCTIONAL"); END;
    IF att.isImplicit THEN Error.Msg ("a variable cannot be IMPLICIT"); END;
...
PROCEDURE Split (v: Value.T;  VAR type: Type.T;
...
PROCEDURE IsGlobal (t: T): M3.Global =
  BEGIN
    IF t = NIL THEN RETURN M3.Global.No;
    ELSIF t.global THEN RETURN M3.Global.Yes;
    ELSIF t.by_reference THEN RETURN M3.Global.Maybe;
    ELSE RETURN t.global_alias; END;
  END IsGlobal;
...
PROCEDURE Check (t: T;  VAR cs: Value.CheckState) =
  ...
  BEGIN
    ...
    IF Type.IsStructured (t.tipe) AND NOT t.indirect THEN
...
PROCEDURE Load (t: T) =
  ...
  BEGIN
    ...
    ELSE (* simple scalar *)
      IF (t.bss_var # NIL) THEN
        CG.Load (t.bss_var, 0, t.size, align, t.cg_type);
      ELSIF (t.indirect) THEN
        CG.Load_addr (v, t.offset);
        CG.Boost_alignment (t.align);
        CG.Load_indirect (t.cg_type, 0, t.size);
      ELSE
        CG.Load (v, t.offset, t.size, align, t.cg_type);
      END;
    END;
  END Load;
...
PROCEDURE CopyOpenArray (t: T;  ref: Type.T) =
  ...
  BEGIN
    ...
    Load (t);
    AssignStmt.Emit (t.tipe, NamedExpr.FromValue (t), M3.Global.Yes,
                     compiled := TRUE);
  ...
  BEGIN
    IF (t.init # NIL) AND (NOT t.initDone) AND (NOT t.imported) THEN
      ...
      ELSE
        ...
        t.initPending := FALSE;
        LoadLValue (t);
        IF IsGlobal (t) = M3.Global.Yes THEN
          EVAL Type.CheckInfo (t.tipe, info);
          AssignStmt.Emit (t.tipe, t.init, M3.Global.Yes);
        ELSE
          AssignStmt.Emit (t.tipe, t.init, M3.Global.No);
        END;
      END;
      t.initDone := TRUE;
      Tracer.Schedule (t.trace);
