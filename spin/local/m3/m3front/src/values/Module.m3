...
(* Last modified on Wed Mar  1 08:44:24 PST 1995 by kalsow     *)
...
VAR (* CONST *)
  builtin_name : ARRAY [0..3] OF M3ID.T := ARRAY [0..3] OF M3ID.T {M3ID.NoID, ..};
  builtin      : ARRAY [0..3] OF T      := ARRAY [0..3] OF T {NIL, ..};
...
VAR
  ...
  HeaderMBuf: M3Buf.T := NIL;(*C header file that contains the info*)(* SPIN *)
...
PROCEDURE NewDefn (name: TEXT;  safe: BOOLEAN;  syms: Scope.T): T =
  ...
  BEGIN
    ...
    IF (builtin[0] = NIL)
      THEN builtin_name[0] := t.name;  builtin[0] := t;
      ELSE IF (builtin[1] = NIL) 
	THEN builtin_name[1] := t.name;  builtin[1] := t;
	ELSE IF (builtin[2] = NIL)
	  THEN builtin_name[2] := t.name;  builtin[2] := t;
	  ELSE builtin_name[3] := t.name;  builtin[3] := t;
	END;
      END;
    END;
    t.safe := safe;
...
PROCEDURE Parse (interfaceOnly : BOOLEAN := FALSE): T =
  ...
  BEGIN
    ...
    IF t.safe AND t.external AND NOT Host.extern_ok THEN
	Error.Msg ("A safe interface may not be <*EXTERNAL*>");
    END;
...
(* instantiate a call on a generic interface or module *)
  ...
  BEGIN
    ...
    rd := Host.OpenUnit (genericName, t.interface, TRUE, FALSE, filename);
    IF (rd = NIL) THEN
...
PROCEDURE LookUp (name: M3ID.T;  internal: BOOLEAN): T =
  ...
  BEGIN
    ...
    ELSE
      ...
      rd := Host.OpenUnit (name, TRUE, FALSE, TRUE, filename);
      IF (rd = NIL) THEN
...
PROCEDURE GetImportedInterface(name: M3ID.T): T =
  BEGIN
    RETURN External.GetImportedInterface(curModule.externals, name);
  END GetImportedInterface;
...
PROCEDURE RecordInterface (t: T) =
  ...
  BEGIN
    ...
    IF (t.name = builtin_name[0]) AND (t # builtin[0]) THEN RETURN END;
    IF (t.name = builtin_name[1]) AND (t # builtin[1]) THEN RETURN END;
    IF (t.name = builtin_name[2]) AND (t # builtin[2]) THEN RETURN END;
    IF (t.name = builtin_name[3]) AND (t # builtin[3]) THEN RETURN END;
...
PROCEDURE SetGlobals (t: T) =
  ...
  BEGIN
    ...
    (* SPIN *)
    IF Host.load_map AND Scanner.in_main AND IsInterface() THEN
      HeaderMBuf := M3Buf.New();
      M3Buf.PutText(HeaderMBuf, "#ifndef __MODULA_3_TYPES__\n");
      M3Buf.PutText(HeaderMBuf, "#define __MODULA_3_TYPES__\n");
      M3Buf.PutText(HeaderMBuf, "typedef char* _ADDRESS;\n");
      M3Buf.PutText(HeaderMBuf, "typedef unsigned long (*_PROC)();\n\n");
      M3Buf.PutText(HeaderMBuf, "typedef struct module {\n");
      M3Buf.PutText(HeaderMBuf, "\t  _ADDRESS  file;\n");
      M3Buf.PutText(HeaderMBuf, "\t  _ADDRESS  type_cells;\n");
      M3Buf.PutText(HeaderMBuf, "\t  _ADDRESS  type_cell_ptrs;\n");
      M3Buf.PutText(HeaderMBuf, "\t  _ADDRESS  full_revelations;\n");
      M3Buf.PutText(HeaderMBuf, "\t  _ADDRESS  partial_revelations;\n");
      M3Buf.PutText(HeaderMBuf, "\t  _ADDRESS  proc_info;\n");
      M3Buf.PutText(HeaderMBuf, "\t  _ADDRESS  proc_types;\n");
      M3Buf.PutText(HeaderMBuf, "\t  _ADDRESS  try_scopes;\n");
      M3Buf.PutText(HeaderMBuf, "\t  _ADDRESS  var_map;\n");
      M3Buf.PutText(HeaderMBuf, "\t  _ADDRESS  gc_map;\n");
      M3Buf.PutText(HeaderMBuf, "\t  _PROC     link;\n");
      M3Buf.PutText(HeaderMBuf, "\t  _PROC     main;\n");
      M3Buf.PutText(HeaderMBuf, "} _MODULE;\n\n");
      M3Buf.PutText(HeaderMBuf, "#endif __MODULA_3_TYPES__\n\n");
      M3Buf.PutText(HeaderMBuf, "extern struct module M" & Prefix(t) &";\n\n");
    ...
    (* SPIN *)
    IF HeaderMBuf # NIL THEN
      TRY
        HeaderFile := FileWr.Open(NameToText(t) & ".h");
        M3Buf.Flush(HeaderMBuf, HeaderFile);
        Wr.Close(HeaderFile);
      EXCEPT
      ELSE 
      END;
...
PROCEDURE Allocate (size, align: INTEGER;
                    tag: TEXT := NIL;  id: M3ID.T := M3ID.NoID;
		    proc : BOOLEAN := FALSE (* SPIN *)
	        ): INTEGER =
  VAR offset: INTEGER;
  BEGIN
    ...
    (* SPIN *)
    IF curModule # NIL AND HeaderMBuf # NIL AND proc THEN
(*
      M3Buf.PutText(HeaderMBuf, 
                    "#define " & NameToText(curModule) & "__" & M3ID.ToText(id) & 
                    "  ((&M" & Prefix(curModule) & 
                           ".main)[(" & Fmt.Int(offset DIV Target.Byte)
                            & " - sizeof(_ADDRESS) * 6)/sizeof(_PROC)])\n");
*)
      M3Buf.PutText(HeaderMBuf, 
                    "#define " & NameToText(curModule) & "__" & M3ID.ToText(id)
		    & "  (*(_PROC*)( (char*)(&M" & Prefix(curModule) & ")+" &
			       	Fmt.Int(offset DIV Target.Byte) 
			       & "))\n");
    END;
...
PROCEDURE CompileInterface (t: T) =
  VAR proc_info, proc_types, type_map, rev_full, rev_part: INTEGER;  
      link_proc: CG.Proc;
  BEGIN
    ...
    ProcBody.EmitAll (proc_info, proc_types, link_proc);
    type_map := Variable.GenGlobalMap (t.localScope);

    GenLinkerInfo (t, proc_info, proc_types, 
                   type_map, rev_full, rev_part, link_proc);
...
PROCEDURE DeclareGlobalData (t: T) =
  BEGIN
    ...
    IF Host.linkinfo_ok THEN
      (* SPIN *)
      CG.Comment (-1, "module link information");
      t.link_info := CG.Declare_segment (
                         M3ID.Add("Link_" & Prefix (t)), ModuleTypeUID);
    END;
...
PROCEDURE GenLinkerInfo (t: T;
                         proc_info, proc_types: INTEGER;
                         type_map, rev_full, rev_part: INTEGER;
                         link_proc: CG.Proc) =
  ...
  VAR
    ...
    link_text: TEXT; (* SPIN *)
    info_len, len, offset: INTEGER;
  BEGIN
    Scanner.offset := t.origin;
    Scanner.Here (file, line);
    line := CG.EmitText (file);
    CG.Init_var (M3RT.MI_file, v, line);
    CG.Comment (line, "file name");
    ...
    END;
    IF (proc_types # 0) THEN
      CG.Init_var (M3RT.MI_proc_types, v, proc_types);
    END;
    ...
    (* SPIN *)
      CG.Comment (-1, "module link information");
      link_text := Host.env.get_link_info_text();
      info_len := Text.Length(link_text);
      CG.Init_intt (0, Target.Int_C.size, info_len);
      len := Target.Int_C.size;
      offset := Target.Int_C.size;
      CG.Init_chars (offset, link_text);
      INC(len, info_len);
      INC(offset, info_len * Target.Char.size);
      CG.Init_intt (offset, Target.Char.size, 0);
      INC(len, Target.Char.size);
      CG.Bind_segment (t.link_info, len, CG.Max_alignment,
                     CG.Type.Struct, exported := TRUE, init := TRUE);
    END;
  END GenLinkerInfo;
...
PROCEDURE NameToText (t: T): TEXT =
  BEGIN
    RETURN M3ID.ToText (Name(t));
  END NameToText;
