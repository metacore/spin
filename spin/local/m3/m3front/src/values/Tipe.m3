 ...
 *	DefineOpaque calls OpaqueType.New with name argument 
 *
 *)
...
PROCEDURE Parse (READONLY att: Decl.Attributes) =
  ...
  BEGIN
    ...
    END;
    IF att.isBounded    THEN Error.Msg ("a type cannot be EPHEMERAL"); END;
    IF att.isFunctional THEN Error.Msg ("a type cannot be FUNCTIONAL"); END;
    IF att.isImplicit THEN Error.Msg ("a type cannot be IMPLICIT"); END;
...
PROCEDURE DefineOpaque (name: TEXT;  super: Type.T; 
			reserved: BOOLEAN := TRUE): Type.T =
  VAR t: T;
  BEGIN
    t := Create (M3ID.Add (name));
    Scope.Insert (t);
    t.value := OpaqueType.New (super, t, name := name);
    IF (reserved) THEN Scanner.NoteReserved (t.name, t); END;
    RETURN t.value;
  END DefineOpaque;

PROCEDURE DefineBuiltinOpaque (name: TEXT;  super: Type.T; 
			       reserved: BOOLEAN := TRUE): Type.T =
  VAR t: T;
  BEGIN
    t := Create (M3ID.Add (name));
    Scope.Insert (t);
    t.value := OpaqueType.New (super, t, TRUE);
    IF (reserved) THEN Scanner.NoteReserved (t.name, t); END;
    RETURN t.value;
  END DefineBuiltinOpaque;
