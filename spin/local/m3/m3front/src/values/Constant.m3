...
(*
 * HISTORY
 * 07-Oct-96  Wilson Hsieh (whsieh) at the University of Washington
 *	grabbed Constant.Check from version 3.6
 *
...
PROCEDURE ParseDecl (READONLY att: Decl.Attributes) =
  ...
  BEGIN
    ...
    END;
    IF att.isBounded    THEN Error.Msg ("a constant cannot be EPHEMERAL"); END;
    IF att.isFunctional THEN Error.Msg ("a constant cannot be FUNCTIONAL"); END;
    IF att.isImplicit THEN Error.Msg ("a constant cannot be IMPLICIT"); END;
