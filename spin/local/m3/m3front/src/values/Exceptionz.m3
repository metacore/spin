...
PROCEDURE ParseDecl (READONLY att: Decl.Attributes) =
  ...
  BEGIN
    ...
    END;
    IF att.isBounded    THEN Error.Msg ("an exception cannot be EPHEMERAL"); END;
    IF att.isFunctional THEN Error.Msg ("an exception cannot be FUNCTIONAL"); END;
...
PROCEDURE EmitBody (x: Raiser) =
  ...
  BEGIN
    ...
    CG.Load_addr (arg);
    CG.Boost_alignment (align);
    IF Host.trace_ref OR Host.ref_count THEN
      (* cannot be an open array, so we can call Type.GenRC *)
      Type.GenRC (t.tipe, TRUE);
    ELSE
