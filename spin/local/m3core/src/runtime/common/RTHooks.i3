 ...
 *	hook for write barrier
 *
 * 27-Jan-97  Wilson Hsieh (whsieh) at the University of Washington
 *	add hook for reference counting
 *	the procedures in this interface cannot have their order changed
 *	 without a synchronized changed to the compiler -- Runtime.?3,
 *)
...
(* the limits of the stack for the currently running thread.
...
(**************  SPIN, for pointer tracking ******************)

PROCEDURE AssignTracedToPossibleGlobal (addr, rhs: ADDRESS);

PROCEDURE AssignTracedToGlobal (addr, rhs: ADDRESS);

<* EXTERNAL *>
PROCEDURE AssignKnown (lhs, rhs: ADDRESS);

PROCEDURE IsShared (a: ADDRESS) : BOOLEAN;
...
PROCEDURE TraceCount (tc: INTEGER (*RT0.Typecode*));
