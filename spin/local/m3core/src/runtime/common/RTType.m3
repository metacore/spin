  ...
  BEGIN
    ...
    FOR i := 0 TO RT0u.nTypes-1 DO
      ...
      IF (t.typecode = LAST (RT0.Typecode)) THEN
        ...
        INC (next_typecode);
      END;
      IF t.subTypeCode = LAST (RT0.Typecode) THEN
        t.subTypeCode := next_subtype;
        t.lastSubTypeCode := next_subtype;
      END;
      INC(next_subtype);
      INC (tp, ADRSIZE (tp^));
    END;
...
PROCEDURE AssignObjectTypecode (t: RT0.TypeDefn;  
  ...
  BEGIN
    ...
    t.lastSubTypeCode := next_subtype-1;
      ...
      IF (r # NIL) THEN
        WHILE (r.lhs_id # 0) DO
          ...
          IF (lhs = NIL) OR (rhs = NIL)
            ...
            OR (rhs.lastSubTypeCode < lhs.subTypeCode) THEN
