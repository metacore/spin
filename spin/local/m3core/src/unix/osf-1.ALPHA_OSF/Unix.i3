...
(*                                                             *)
(* Last modified on Tue Feb  7 15:13:03 PST 1995 by kalsow     *)
(*      modified on Tue Jun  8 16:26:49 PDT 1993 by mcjones    *)
(*      modified on Wed Apr  7 17:35:16 PDT 1993 by muller     *)
...
FROM Utypes IMPORT u_short, dev_t, off_t, ssize_t;
...
CONST
  FNDELAY   = O_NDELAY;       (* non-blocking I/O *)
  FAPPEND   = O_APPEND;       (* append on each write *)
  FASYNC    = 8_000100;       (* signal pgrp when data ready *)
  FCREAT    = O_CREAT;        (* create if nonexistant *)
  FTRUNC    = O_TRUNC;        (* truncate to zero length *)
  FSYNC     = O_SYNC;         (* write file syncronously *)
  FEXCL     = O_EXCL;         (* error if already created *)
  FNONBLOCK = O_NONBLOCK;     (* POSIX non-blocking I/O *)
  FDEFER    = O_DEFER;        (* defer for next gc pass *)
...
<*EXTERNAL*> PROCEDURE brk (addr: void_star): int;
<*EXTERNAL*> PROCEDURE sbrk (inc: ssize_t): void_star;
...
CONST (* l_type values *)
  ...
  F_UNLCK = 8; (* Remove lock(s) *)

<*EXTERNAL*> PROCEDURE fcntl (fd, request: int; arg: long): int;
(* What crap!  The man page says arg is either an "int" or a "struct flock*",
...
(* if we had the structure declarations, we would use these 
  INT    = Shift (And (BYTESIZE (int),                  IOCPARM_MASK), 16);
...
CONST (* the tty i/o controls *)
  ...
  SIOCDIFADDR   =  Or (Or (W, IFR),  Or (IC, 25));  (* delete IF addr *)

  SIOCRDZCTRS =    Or (Or (RW, CTR), Or (IC, 27));  (*  Read/0 if c. *)
  SIOCRDCTRS =     Or (Or (RW, CTR), Or (IC, 28));  (*  Read if cntr.*)
  ...
  SIOCADDMULTI =   Or (Or (RW, IFR), Or (IC, 49));  (*  Add m.c. ad. *)
  SIOCDELMULTI =   Or (Or (RW, IFR), Or (IC, 50));  (*  Dele. m.c.ad.*)

  SIOCENABLBACK =  Or (Or (W, IFR),  Or (IC, 60));  (* Set in.ex.lb. *)
  SIOCDISABLBACK = Or (Or (W, IFR),  Or (IC, 61));  (* Cl.in.ex.lpb. *)
  SIOCRPHYSADDR =  Or (Or (RW, IFD), Or (IC, 62));  (*  Read phy. ad.*)
  SIOCSPHYSADDR =  Or (Or (RW, IFR), Or (IC, 63));  (*  Set phys. ad.*)
...
<*EXTERNAL*> PROCEDURE ioctl (d: int; request: long; argp: ADDRESS): int;
...
CONST (* flags *)
  O_RDONLY   = 8_000000;       (* open for reading *)
  O_WRONLY   = 8_000001;       (* open for writing *)
  O_RDWR     = 8_000002;       (* open for read & write *)
  O_NONBLOCK = 8_000004;       (* POSIX non-blocking I/O *)
  O_APPEND   = 8_000010;       (* append on each write *)
  O_DEFER    = 8_000040;       (* deferred update *)
  O_CREAT    = 8_001000;       (* open with file create *)
  O_TRUNC    = 8_002000;       (* open with truncation *)
  O_EXCL     = 8_004000;       (* exclusive open *)
  O_NOCTTY   = 8_010000;       (* POSIX required ? *)
  O_SYNC     = 8_040000;       (* syncronous write *)
  O_NDELAY   = 8_100000;       (* non-blocking I/O *)
