...
(*                                                             *)
(* Last modified on Mon Oct 24 15:19:41 PDT 1994 by kalsow     *)
...
(*
 * Due to a bug in -lsocket, we use a wrapper for the real fcntl.
 * This will need to be revisted when the port use Solaris-2 threads
 * is complete.
 *
 * Geoff Wyant 9/13/94
 *)
...
<*EXTERNAL "fcntl" *> PROCEDURE raw_fcntl (fd, request, arg: int): int;
(**** actual C entry-point, used to implement the patched version above ****)
...
(*
 * This is another workaround for 'libsocket'.
 *
 * Geoff Wyant - 9/13/94
 *)
...
<*EXTERNAL "ioctl"*> PROCEDURE raw_ioctl (d, request: int; argp: ADDRESS): int;
(**** actual C entry-point, used to implement the patched version above ****)
