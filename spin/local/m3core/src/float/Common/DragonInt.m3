...
(*                                                                           *)
(* Last modified on Fri Jun 17 16:30:50 PDT 1994 by kalsow                   *)
...
REVEAL
  Session = BRANDED "DragonInt.Session" REF RECORD
    next : INTEGER;
    w    : REF ARRAY OF Word.T;
  END;

PROCEDURE NewSession (): Session =
  VAR s := NEW (Session);
  BEGIN
    s.w := NEW (REF ARRAY OF Word.T, 1000);
    s.w[0] := 0; (* Zero *)
    s.w[1] := 1; (* One  *)
    s.next := 2;
    RETURN s;
...
PROCEDURE EndSession (<*UNUSED*> s: Session) =
  BEGIN
    (***
...
PROCEDURE Expand (s: Session) =
  VAR
    n   := NUMBER (s.w^);
    new := NEW (REF ARRAY OF Word.T, n + n);
  BEGIN
