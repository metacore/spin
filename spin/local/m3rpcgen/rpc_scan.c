	...
	default:
		if (!(isalpha(*CurrentContext->where) ||
			...
			if (isprint(*CurrentContext->where)) {
				s_print(p, "%c", *CurrentContext->where);
			} else {
				s_print(p, "%d", *CurrentContext->where);
			}
			error(buf);
		}
	...
	token *tokp;
{
	...
	*mark = str + len;
	...
	(void) strncpy(nameBuf, str, len);
	nameBuf[len] = 0;
	str = StripPrefix(svcName, nameBuf);
	...
	char *line;
{
     ...
     *      should import all the type decls from the corresponding M3
     *      interface file.
     */
	...
	if (p1 == NULL) {
	    fprintf(stderr,
		    ...
		    line0);
	    fprintf(stderr, "  Unable to translate to Modula3.\n");
	    exit(-1);
	}
